...
public class MediaBindingBean {
  ...
  public String getName() {
    if (infoObject instanceof Episode) return getEpisode().getSeriesName();
    else if (infoObject instanceof Movie) return getMovie().getName();
    else if (infoObject instanceof AudioTrack)
      return getAlbumArtist() != null ? getAlbumArtist() : getArtist();
    else if (infoObject instanceof File) return FileUtilities.getName((File) infoObject);
  ...
  public Integer getYear() throws Exception {
    if (infoObject instanceof Episode) return getStartDate().getYear();
    if (infoObject instanceof Movie) return getMovie().getYear();
  ...
  public List<Integer> getEpisodeNumbers() {
    return getEpisodes().stream()
        .map(
            it -> {
              return it.getEpisode() == null
                  ? it.getSpecial() == null ? null : it.getSpecial()
                  : it.getEpisode();
            })
  ...
  public String getE00() {
    ...
    else return "Special " + join(getEpisodeNumbers(), "-");
  }

  @Define("sxe")
  public String getSxE() {
    return EpisodeFormat.SeasonEpisode.formatSxE(
        getSeasonEpisode()); // try to convert absolute numbers to SxE numbers
  }

  @Define("s00e00")
  public String getS00E00() {
    return EpisodeFormat.SeasonEpisode.formatS00E00(
        getSeasonEpisode()); // try to convert absolute numbers to SxE numbers
  }
  ...
  public String getTitle() {
    ...
    if (infoObject instanceof Episode) {
      t =
          infoObject instanceof MultiEpisode
              ? EpisodeFormat.SeasonEpisode.formatMultiTitle(getEpisodes())
              : getEpisode().getTitle(); // implicit support for multi-episode title formatting
  ...
  public SimpleDate getReleaseDate() {
    if (infoObject instanceof Episode) return getEpisode().getAirdate();
    if (infoObject instanceof Movie) return getMovieInfo().getReleased();
    if (infoObject instanceof AudioTrack) return getMusic().getAlbumReleaseDate();
    if (infoObject instanceof File) return new SimpleDate(getTimeStamp());
    ...
    if (releaseDate != null) {
      ...
      long days =
          ChronoUnit.DAYS.between(
              releaseDate.toLocalDate().atStartOfDay(ZoneOffset.UTC).toInstant(), Instant.now());
  ...
  public SimpleDate getStartDate() throws Exception {
    ...
    return getEpisodeList().stream()
        .filter(e -> isRegular(e))
        .map(Episode::getAirdate)
        .filter(Objects::nonNull)
        .min(SimpleDate::compareTo)
        .get();
  }
  ...
  public List<String> getAliasNames() {
    if (infoObject instanceof Movie) return asList(getMovie().getAliasNames());
    if (infoObject instanceof Episode) return getSeriesInfo().getAliasNames();
  ...
  public String getPrimaryTitle() {
    if (infoObject instanceof Movie) return getPrimaryMovieInfo().getOriginalName();
    if (infoObject instanceof Episode)
      return getPrimarySeriesInfo()
          .getName(); // force English series name for TheTVDB data or default to SeriesInfo name
    // (for AniDB episode data this would be the primary title)
  ...
  public Object getId() throws Exception {
    if (infoObject instanceof Episode) return getEpisode().getSeriesInfo().getId();
    if (infoObject instanceof Movie) return getMovie().getId();
    if (infoObject instanceof AudioTrack) return getMusic().getMBID();
  ...
  public String getTmdbId() {
    if (getMovie().getTmdbId() > 0) return String.valueOf(getMovie().getTmdbId());
  ...
  public String getImdbId() {
    if (getMovie().getImdbId() > 0) return String.format("tt%07d", getMovie().getImdbId());
  ...
  public String getAudioChannels() {
    ...
    int ch =
        tokenize(channels, SLASH)
            .map(s -> matchInteger(s))
            .filter(Objects::nonNull)
            .min(Integer::compare)
            .get();
  ...
  public String getAudioChannelPositions() {
    String channels =
        getMediaInfo(
            StreamKind.Audio, 0, "ChannelPositions/String2", "Channel(s)_Original", "Channel(s)");

    // e.g. ChannelPositions/String2: 3/2/2.1 / 3/2/0.1 (one audio stream may contain multiple
  ...
  public String getAudioChannelObjects() {
    return getMediaInfo(StreamKind.Audio, "Codec_Profile", "Format_Profile", "Format_Commercial")
        .filter(Objects::nonNull)
        .map(
            s -> {
  ...
  public String getHighDynamicRange() {
    if (getVideoBitDepth() >= 10
        && getMediaInfo(StreamKind.Video, 0, "colour_primaries").matches("BT.2020")) {
      return "HDR";
    }
    return null;
  }
  ...
  public String getVideoDefinitionCategory() {
    ...
    // UHD
    if (dim.get(0) >= 3840 || dim.get(1) >= 2160) return "UHD";

    // HD
    if (dim.get(0) >= 1280 || dim.get(1) >= 720) return "HD";
  ...
  public List<Integer> getDimension() {
    ...
    return Stream.of(StreamKind.Video, StreamKind.Image)
        .map(
            k -> {
              // collect Width and Height as Integer List
              return Stream.of("Width", "Height")
                  .map(p -> getMediaInfo().get(k, 0, p))
                  .filter(s -> s.length() > 0)
  ...
  public String getCRC32() throws Exception {
    ...
    Optional<String> embeddedChecksum =
        stream(getFileNames(inferredMediaFile))
            .map(Normalization::getEmbeddedChecksum)
            .filter(Objects::nonNull)
            .findFirst();
  ...
  public String getReleaseGroup() throws Exception {
    ...
    Pattern[] nonGroupPattern = {
      getKeywordExcludePattern(),
      releaseInfo.getVideoSourcePattern(),
      releaseInfo.getVideoFormatPattern(true),
      releaseInfo.getResolutionPattern(),
      releaseInfo.getStructureRootPattern()
    };

    // consider foldername, filename and original filename of inferred media file
    String[] filenames =
        stream(mediaTitles.get())
            .map(s -> releaseInfo.clean(s, nonGroupPattern))
            .filter(s -> s.length() > 0)
            .toArray(String[]::new);
  ...
  public Integer getRuntime() {
    if (infoObject instanceof Movie) return getMovieInfo().getRuntime();
    if (infoObject instanceof Episode) return getSeriesInfo().getRuntime();
  ...
  public List<String> getActors() throws Exception {
    if (infoObject instanceof Movie) return getMovieInfo().getActors();
    if (infoObject instanceof Episode)
      return ExpressionFormatMethods.getActors(
  ...
  public List<String> getGenres() {
    if (infoObject instanceof Movie) return getMovieInfo().getGenres();
    if (infoObject instanceof Episode) return getSeriesInfo().getGenres();
  ...
  public String getDirector() throws Exception {
    if (infoObject instanceof Movie) return getMovieInfo().getDirector();
    if (infoObject instanceof Episode)
      return ExpressionFormatMethods.getInfo(getEpisode())
          .getDirector(); // use TheTVDB API v2 to retrieve extended episode info
  ...
  public String getCertification() {
    if (infoObject instanceof Movie) return getMovieInfo().getCertification();
    if (infoObject instanceof Episode) return getSeriesInfo().getCertification();
  ...
  public Double getRating() {
    if (infoObject instanceof Movie) return getMovieInfo().getRating();
    if (infoObject instanceof Episode) return getSeriesInfo().getRating();
  ...
  public Integer getVotes() {
    if (infoObject instanceof Movie) return getMovieInfo().getVotes();
    if (infoObject instanceof Episode) return getSeriesInfo().getRatingCount();
  ...
  public String getCollection() {
    if (infoObject instanceof Movie) return getMovieInfo().getCollection();
  ...
  public synchronized AssociativeScriptObject getMetaInfo() {
    if (infoObject instanceof Movie) return createPropertyBindings(getMovieInfo());
    if (infoObject instanceof Episode) return createPropertyBindings(getSeriesInfo());
  ...
  public List<Integer> getSeasonYears() throws Exception {
    return getEpisodeList().stream()
        .filter(
            e -> isRegular(e) && e.getSeason().equals(getSeasonNumber()) && e.getAirdate() != null)
        .map(e -> e.getAirdate().getYear())
        .sorted()
        .distinct()
        .collect(toList());
  }
  ...
  public Integer getSeasonCount() throws Exception {
    return getEpisodeList().stream()
        .filter(e -> isRegular(e) && e.getSeason() != null)
  ...
  public List<Language> getAudioLanguageList() {
    return getMediaInfo(StreamKind.Audio, "Language")
        .filter(Objects::nonNull)
        .distinct()
        .map(Language::findLanguage)
        .filter(Objects::nonNull)
  ...
  public List<Language> getTextLanguageList() {
    return getMediaInfo(StreamKind.Text, "Language")
        .filter(Objects::nonNull)
        .distinct()
        .map(Language::findLanguage)
        .filter(Objects::nonNull)
  ...
  public Integer getPart() {
    if (infoObject instanceof AudioTrack) return getMusic().getTrack();
    if (infoObject instanceof MoviePart) return ((MoviePart) infoObject).getPartIndex();
  ...
  public Integer getPartCount() {
    if (infoObject instanceof AudioTrack) return getMusic().getTrackCount();
    if (infoObject instanceof MoviePart) return ((MoviePart) infoObject).getPartCount();
  ...
  public String getUserDefinedLabel() throws IOException {
    return getUserDefinedArguments().entrySet().stream()
        .filter(
            it -> {
              return it.getKey().endsWith("label")
                  && it.getValue() != null
                  && it.getValue().length() > 0;
            })
        .map(it -> it.getValue())
        .findFirst()
        .orElse(null);
  ...
  public Integer getDuplicateIndex() {
    return 1
        + identityIndexOf(
            context.values().stream().filter(getInfoObject()::equals).collect(toList()),
            getInfoObject());
  }
  ...
  public File getInferredMediaFile() {
    ...
    if (file.isDirectory()) {
    ...
    } else if ((SUBTITLE_FILES.accept(file) || IMAGE_FILES.accept(file))
        || ((infoObject instanceof Episode || infoObject instanceof Movie)
            && !VIDEO_FILES.accept(file))) {
      ...
      for (File movieFile : videos) {
        if (!baseName.isEmpty()
            && stripReleaseInfo(FileUtilities.getName(movieFile))
                .toLowerCase()
                .startsWith(baseName)) {
          return movieFile;
        }
      ...
      if (videos.size() > 0) {
        sort(
            videos,
            SimilarityComparator.compareTo(FileUtilities.getName(file), FileUtilities::getName));
        return videos.get(0);
      }
    }
  ...
  public Episode getSeasonEpisode() {
    // magically convert AniDB absolute numbers to TheTVDB SxE numbers if AniDB is selected with
    // airdate SxE episode sort order
  ...
  public SeriesInfo getPrimarySeriesInfo() {
    if (TheTVDB.getIdentifier().equals(getSeriesInfo().getDatabase())) {
      try {
        ...
        debug.warning(
            "Failed to retrieve primary series info: " + e); // default to seriesInfo property
      }
    }
  ...
  private final Resource<MovieInfo> primaryMovieInfo =
      Resource.lazy(() -> TheMovieDB.getMovieInfo(getMovie(), Locale.US, false));
  private final Resource<MovieInfo> extendedMovieInfo =
      Resource.lazy(() -> getMovieInfo(getMovie().getLanguage(), true));
  ...
  public synchronized MovieInfo getMovieInfo(Locale locale, boolean extendedInfo) throws Exception {
    ...
    if (m.getImdbId() > 0) return OMDb.getMovieInfo(m);

    return null;
  }

  private static final Cache<File, MediaInfo> mediaInfoCache =
  ...
  private Stream<String> getMediaInfo(StreamKind streamKind, String... keys) {
    return IntStream.range(0, getMediaInfo().streamCount(streamKind))
        .mapToObj(
            streamNumber -> {
              return stream(keys)
                  .map(
  ...
  private AssociativeScriptObject createBindingObject(
      File file, Object info, Map<File, ?> context, Function<String, Object> defaultValue) {
    MediaBindingBean mediaBindingBean =
        new MediaBindingBean(info, file, context) {
  ...
  private List<AssociativeScriptObject> createMediaInfoBindings(StreamKind kind) {
    return getMediaInfo().snapshot().get(kind).stream()
        .map(m -> new AssociativeScriptObject(m, this::undefined))
        .collect(toList());
  }

  private final Resource<String[]> mediaTitles =
      Resource.lazy(
          () -> {
            ...
            try {
              ...
              if (mediaTitle.length() > 0) {
                return Stream.concat(
                        Stream.of(mediaTitle), stream(getFileNames(getInferredMediaFile())))
                    .toArray(String[]::new);
              }
            } catch (Exception e) {
  ...
  public static final String EXCEPTION_SAMPLE_FILE_NOT_SET =
      "Sample file has not been set. Click \"Change Sample\" to select a sample file.";
}
