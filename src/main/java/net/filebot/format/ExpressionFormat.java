...
public class ExpressionFormat extends Format {
        ...
        if (level == 1) {
          if (token.length() > 0) {
            try {
              ...
              try {
                String message =
                    findCause(e, MultipleCompilationErrorsException.class)
                        .getErrorCollector()
                        .getSyntaxError(0)
                        .getOriginalMessage();
  ...
  public String format(Bindings bindings) {
    ...
    Bindings priviledgedBindings =
        PrivilegedInvocation.newProxy(Bindings.class, bindings, AccessController.getContext());
    ...
    this.suppressed =
        suppressed.isEmpty() ? null : new SuppressedThrowables("Suppressed", suppressed);
  ...
  protected Throwable normalizeExpressionException(ScriptException exception) {
    if (findCause(exception, MissingPropertyException.class) != null) {
      return new BindingException(
          findCause(exception, MissingPropertyException.class).getProperty(),
          "undefined",
          exception);
    }
    ...
    if (findCause(exception, GroovyRuntimeException.class) != null) {
      return new ExpressionException(
          findCause(exception, GroovyRuntimeException.class).getMessage(), exception);
    }
