...
public final class VerificationUtilities {
  ...
  /**
   * A {@link Pattern} that will match checksums enclosed in brackets ("[]" or "()"). A checksum
   * string is a hex number with at least 8 digits. Capturing group 0 will contain the matched
   * checksum string.
   */
  public static final Pattern EMBEDDED_CHECKSUM =
      Pattern.compile("(?<=\\[|\\()(\\p{XDigit}{8})(?=\\]|\\))");
  ...
  public static String getHashFromVerificationFile(File file, HashType type, int maxDepth)
      throws IOException {
    return getHashFromVerificationFile(file.getParentFile(), file, type, 0, maxDepth);
  }

  private static String getHashFromVerificationFile(
      File folder, File target, HashType type, int depth, int maxDepth) throws IOException {
  ...
  public static String computeHash(File file, HashType type)
