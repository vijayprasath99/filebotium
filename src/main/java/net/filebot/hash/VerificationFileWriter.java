...
public class VerificationFileWriter implements Closeable {
  ...
  public VerificationFileWriter(File file, VerificationFormat format, Charset charset)
      throws IOException {
    this(
        new PrintWriter(
            new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), charset)), false),
        format,
        charset);
  }
  ...
  protected void writeHeader(Charset charset) {
    out.format(
        "; Generated by %s %s on %tF at %<tT%n",
        Settings.getApplicationName(), Settings.getApplicationVersion(), new Date());
