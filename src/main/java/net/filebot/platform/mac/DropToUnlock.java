...
public class DropToUnlock extends JList<File> {
  ...
  public static final Map<String, String> persistentSecurityScopedBookmarks =
      Settings.forPackage(DropToUnlock.class).node("SecurityScopedBookmarks").asMap();
  ...
  public static void unlockBySecurityScopedBookmarks(List<File> folders) {
    synchronized (persistentSecurityScopedBookmarks) {
      ...
      for (File folder : folders) {
        Optional<File> bookmarkForFolder =
            listPath(folder).stream().filter(f -> bookmarks.contains(f.getPath())).findFirst();
        if (bookmarkForFolder.isPresent() && isLockedFolder(folder)) {
          try {
            NSURL_URLByResolvingBookmarkData_startAccessingSecurityScopedResource(
                persistentSecurityScopedBookmarks.get(bookmarkForFolder.get().getPath()));
          } catch (Throwable e) {
            debug.severe(
                "NSURL.URLByResolvingBookmarkData.startAccessingSecurityScopedResource: " + e);
          }
        }
      }
    }
  ...
  public static List<File> getParentFolders(Collection<File> files) {
    return files.stream()
        ...
        .sorted()
        .distinct()
        .filter(f -> !f.exists() || isLockedFolder(f))
        .map(
            f -> {
              try {
                ...
                if (root == null
                    || root.getName().isEmpty()
                    || root.getParentFile() == null
                    || root.getParentFile().getName().isEmpty()) {
        ...
        .filter(f -> f != null && !f.getName().isEmpty() && isLockedFolder(f))
        .sorted()
        .distinct()
  ...
  public static boolean showUnlockFoldersDialog(Window owner, Collection<File> files) {
    ...
    // try to restore permissions from previously stored security-scoped bookmarks as best as
    // possible
    ...
    RunnableFuture<Boolean> showPermissionDialog =
        new FutureTask<Boolean>(
            () -> {
                    ...
                    public void updateLockStatus(File... folders) {
                      ...
                      if (model.stream().allMatch(f -> !isLockedFolder(f))) {
                        ...
                        invokeLater(
                            ...
                            () ->
                                dialog.setVisible(
                                    false)); // auto-close unlock dialog once all folders have been
                        // unlocked
                        invokeLater(
                            1000,
                            () ->
                                Desktop.getDesktop()
                                    .requestForeground(
                                        true)); // bring application to foreground now that folders
                        // have been unlocked
                      } else {
                        model.stream()
                            .filter(f -> isLockedFolder(f))
                            .findFirst()
                            .ifPresent(
                                f -> {
                                  invokeLater(
              ...
              JLabel help =
                  new JLabel(
                      "<html>FileBot does not have permission to access the folder above. To allow FileBot access, drag and drop the folder from Finder onto the drop area above</b>. The permissions for this folder (and all the folders it contains) will be remembered and FileBot will not need to ask for it again.</html>");
              help.setBorder(
                  createCompoundBorder(
                      new SeparatorBorder(
                          ...
                          Position.TOP),
                      createTitledBorder("About App Sandboxing")));
              ...
              // open required folders for easy drag and drop (a few milliseconds after the dialog
              // has become visible)
              invokeLater(
                  500,
                  () -> {
        ...
        .forEach(
            f -> {
              try {
                ...
                log.log(
                    Level.SEVERE, format("Permission denied: %s (%s %s)", f, permissions, owner));
              } catch (Exception e) {
  ...
  protected void paintComponent(Graphics g) {
    ...
    dropArea.setFrameFromCenter(
        getWidth() / 2,
        getHeight() - (h / 2) - pad - 10,
        (getWidth() - w) / 2,
        getHeight() - h - 2 * pad);
    ...
    g2d.drawString(
        "to Unlock 'em", (int) dropArea.getMinX() + 45, (int) dropArea.getMinY() + 40 + 35);
  }
  ...
  protected static class FolderLockCellRenderer extends DefaultListCellRenderer {
    ...
    public Component getListCellRendererComponent(
      ...
      JLabel c =
          (JLabel) super.getListCellRendererComponent(list, folder.getName(), index, false, false);
          ...
          if (isLockedFolder(folder)) {
            if (null
                != showOpenDialogSelectFolder(
                    folder,
                    "Grant Permission",
                    new ActionEvent(list, ActionEvent.ACTION_PERFORMED, "Grant"))) {
