...
public class MacAppUtilities {
  ...
  public static String NSURL_bookmarkDataWithOptions(String path) {
    return objc()
        .sendProxy("NSURL", "fileURLWithPath:", path)
        .sendProxy(
            ...
            null,
            null)
        .sendString("base64Encoding");
  }

  public static Object NSURL_URLByResolvingBookmarkData_startAccessingSecurityScopedResource(
      String text) {
    return objc()
        .sendProxy(
            "NSURL",
            "URLByResolvingBookmarkData:options:relativeToURL:bookmarkDataIsStale:error:",
            NSData_initWithBase64Encoding(text),
            1024,
            null,
            false,
            null)
        .send("startAccessingSecurityScopedResource");
  }

  public static List<File> NSOpenPanel_openPanel_runModal(
      String title,
    ...
    // WARNING: dispatch_sync seems to work on most Mac always causes a deadlock and freezes the
    // application on others (in particular MBP with 2 graphics chips)
    dispatch_async(
        () -> {
          ...
          if (allowedFileTypes != null) {
            Proxy mutableArray =
                objc().sendProxy("NSMutableArray", "arrayWithCapacity:", allowedFileTypes.length);
  ...
  public static void initializeApplication(
      JMenuBar appMenuBar, Consumer<List<File>> openFileHandler) {
  ...
  public static boolean isLockedFolder(File folder) {
    // write permissions my not be available even after sandbox has granted permission (e.g. when
    // accessing files of another user)
