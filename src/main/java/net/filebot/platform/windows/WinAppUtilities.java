...
public class WinAppUtilities {
  ...
  public static String getAppUserModelID() {
    try {
      ...
      if (Shell32.INSTANCE
          .GetCurrentProcessExplicitAppUserModelID(ppszAppID)
          .equals(WinError.S_OK)) {
  ...
  public static String getPackageName() {
    ...
    LPWSTR packageFullName =
        new LPWSTR(new Memory(packageFullNameLength.getValue().intValue() * Native.WCHAR_SIZE));

    NativeLong r =
        Kernel32.INSTANCE.GetCurrentPackageFullName(packageFullNameLength, packageFullName);
  ...
  public static String getPackageAppUserModelID() {
    ...
    LPWSTR applicationUserModelId =
        new LPWSTR(
            new Memory(applicationUserModelIdLength.getValue().intValue() * Native.WCHAR_SIZE));

    NativeLong r =
        Kernel32.INSTANCE.GetCurrentApplicationUserModelId(
            applicationUserModelIdLength, applicationUserModelId);

    if (r.intValue() != W32Errors.ERROR_SUCCESS) {
      throw new IllegalStateException(
          String.format("Kernel32.GetCurrentApplicationUserModelId (%s)", r));
    }
  ...
  public static void initializeApplication(String aumid) {
    ...
    UIManager.put(
        ...
        createCompoundBorder(
            createLineBorder(new Color(0xD7D7D7), 1, true),
            createCompoundBorder(
                createMatteBorder(6, 5, 6, 5, new Color(0xE5E5E5)),
