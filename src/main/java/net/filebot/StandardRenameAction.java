...
public enum StandardRenameAction implements RenameAction {
    ...
    public File rename(File from, File to) throws Exception {
      ...
      if (Platform.isMac()) {
        ...
        process.command(
            ...
            "--reflink",
            "--force",
            from.isDirectory() ? "--recursive" : "--no-target-directory",
            from.getPath(),
            dest.getPath());
      }
  ...
  public static File revert(File current, File original) throws IOException {
    ...
    throw new IllegalArgumentException(
        String.format("Cannot revert file: %s => %s", current, original));
  }
}
