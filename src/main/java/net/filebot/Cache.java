...
public class Cache {
  ...
  public <T> CachedResource<T, byte[]> bytes(T key, Transform<T, URL> resource) {
    return new CachedResource<T, byte[]>(
        key, resource, fetchIfModified(), getBytes(), byte[].class::cast, ONE_DAY, this);
  }

  public <T> CachedResource<T, byte[]> bytes(
      T key, Transform<T, URL> resource, Transform<InputStream, InputStream> decompressor) {
    return new CachedResource<T, byte[]>(
        key,
        resource,
        fetchIfModified(),
        getBytes(decompressor),
        byte[].class::cast,
        ONE_DAY,
        this);
  }
  ...
  public <T> CachedResource<T, String> text(T key, Transform<T, URL> resource) {
    return new CachedResource<T, String>(
        key, resource, fetchIfModified(), getText(UTF_8), String.class::cast, ONE_DAY, this);
  }

  public <T> CachedResource<T, Document> xml(T key, Transform<T, URL> resource) {
    return new CachedResource<T, Document>(
        key,
        resource,
        fetchIfModified(),
        validateXml(getText(UTF_8)),
        getXml(String.class::cast),
        ONE_DAY,
        this);
  }

  public <T> CachedResource<T, Object> json(T key, Transform<T, URL> resource) {
    return new CachedResource<T, Object>(
        key,
        resource,
        fetchIfModified(),
        validateJson(getText(UTF_8)),
        getJson(String.class::cast),
        ONE_DAY,
        this);
  }
  ...
  public Object computeIf(Object key, Predicate<Element> condition, Compute<?> compute)
      throws Exception {
    // get if present
  ...
  public static Predicate<Element> isStale(Duration expirationTime) {
    return element ->
        System.currentTimeMillis() - element.getLatestOfCreationAndUpdateTime()
  ...
  public <V> TypedCache<List<V>> castList(Class<V> cls) {
    return new TypedCache<List<V>>(
        cache,
        cacheType,
        it -> it == null ? null : stream((Object[]) it).map(cls::cast).collect(toList()),
        it -> it == null ? null : it.toArray());
  }
  ...
  public static class TypedCache<V> extends Cache {
    ...
    public TypedCache(
        ...
        CacheType cacheType,
        Function<Object, V> read,
        Function<V, Object> write) {
    ...
    @Override
    public V computeIf(Object key, Predicate<Element> condition, Compute<?> compute)
        throws Exception {
