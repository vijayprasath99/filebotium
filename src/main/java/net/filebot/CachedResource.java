...
public class CachedResource<K, R> implements Resource<R> {
  ...
  public CachedResource(
      K key,
      Transform<K, URL> resource,
      Fetch fetch,
      Transform<ByteBuffer, ? extends Object> parse,
      Transform<? super Object, R> cast,
  ...
  public synchronized R get() throws Exception {
    Object value =
        cache.computeIf(
            ...
            element -> {
              ...
              try {
                ByteBuffer data =
                    retry(() -> fetch.fetch(url, lastModified), retryLimit, retryWait);
    ...
    try {
      ...
      throw new IllegalStateException(
          String.format("Failed to cast cached value: %s => %s (%s)", key, value, cache), e);
    }
  }

  protected <T> T retry(Callable<T> callable, int retryCount, Duration retryWaitTime)
      throws Exception {
    try {
      ...
      debug.warning(
          format(
              "Fetch failed: Try again in %d seconds (%d more) => %s",
              retryWaitTime.getSeconds(), retryCount, e));
  ...
  public static Transform<ByteBuffer, byte[]> getBytes(
      Transform<InputStream, InputStream> decompressor) {
    return data -> {
  ...
  public static Fetch fetchIfNoneMatch(Transform<URL, ?> key, Cache cache) {
    ...
    return fetchIfNoneMatch(
        url -> {
          try {
            return cache.get(key.transform(url)) == null
                ? null
                : etagStorage.get(key.transform(url));
          } catch (Exception e) {
            throw new RuntimeException(e);
          }
        },
  ...
  public static Fetch fetchIfNoneMatch(
      Function<URL, Object> etagRetrieve, BiConsumer<URL, String> etagStore) {
    return (url, lastModified) -> {
      Object etagValue = etagRetrieve.apply(url);
      debug.fine(WebRequest.log(url, lastModified, etagValue));
      try {
        return WebRequest.fetch(
            url,
            etagValue == null ? lastModified : 0,
            etagValue,
            null,
            storeETag(url, etagStore, etag -> !etag.equals(etagValue)));
      } catch (FileNotFoundException e) {
        return fileNotFound(url, e);
      }
    };
  ...
  private static Consumer<Map<String, List<String>>> storeETag(
      URL url, BiConsumer<URL, String> etagStore, Predicate<String> etagFilter) {
    return responseHeaders -> {
      WebRequest.getETag(responseHeaders)
          .filter(etagFilter)
          .ifPresent(
              etag -> {
