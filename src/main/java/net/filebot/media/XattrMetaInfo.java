...
public class XattrMetaInfo {
  ...
  public static final XattrMetaInfo xattr =
      new XattrMetaInfo(useExtendedFileAttributes(), useCreationDate());

  private final boolean useExtendedFileAttributes;
  private final boolean useCreationDate;

  private final Cache<File, Optional<Object>> xattrMetaInfoCache =
      Caffeine.newBuilder().expireAfterAccess(24, TimeUnit.HOURS).build();
  private final Cache<File, Optional<Object>> xattrOriginalNameCache =
      Caffeine.newBuilder().expireAfterAccess(24, TimeUnit.HOURS).build();
    ...
    if (object instanceof Episode) {
      ...
      if (movie.getYear() > 0 && movie.getTmdbId() > 0) {
        SimpleDate releaseDate =
            WebServices.TheMovieDB.getMovieInfo(movie, Locale.US, false).getReleased();
        if (releaseDate != null) {
  ...
  private Object getXattrValue(
      Cache<File, Optional<Object>> cache, File file, Function<MetaAttributes, Object> compute) {
