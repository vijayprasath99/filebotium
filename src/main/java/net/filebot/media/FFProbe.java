...
public class FFProbe implements MediaCharacteristics {
    ...
    Map<String, Object> json =
        (Map)
            JsonReader.jsonToJava(
                process.getInputStream(), singletonMap(JsonReader.USE_MAPS, true));

    int exitCode = process.waitFor();
    if (exitCode != 0) {
      throw new IOException(
          String.format("%s failed with exit code %d", processBuilder.command(), exitCode));
    }
  ...
  public Float getFrameRate() {
    return find("video", "avg_frame_rate")
        .map(
            fps -> {
              switch (fps) {
                case "500/21":
  ...
  protected String getString(String streamKind, String objectKey, String valueKey) {
    return stream(streamKind, objectKey)
        .map(t -> ((Map) t).get(valueKey))
  ...
  protected Stream<Object> stream(String streamKind, String property) {
    return getStreams().stream()
        .filter(s -> streamKind.equals(s.get("codec_type")))
