...
public class ImageMetadata {
  ...
  public Map<String, String> snapshot(Function<Tag, String> key) {
    return snapshot(
        key,
        d ->
            Stream.of("JPEG", "JFIF", "Interoperability", "Huffman", "File")
                .noneMatch(d.getName()::equals));
  }
  ...
  public Optional<String> getName() {
    return extract(m -> m.getFirstDirectoryOfType(FileSystemDirectory.class))
        .map(d -> d.getString(FileSystemDirectory.TAG_FILE_NAME));
  }

  public Optional<ZonedDateTime> getDateTaken() {
    return extract(m -> m.getFirstDirectoryOfType(ExifIFD0Directory.class))
        .map(d -> d.getDate(ExifSubIFDDirectory.TAG_DATETIME))
        .map(
            d -> {
    ...
    return extract(m -> m.getFirstDirectoryOfType(ExifIFD0Directory.class))
        .map(
            d -> {
              ...
              Map<CameraProperty, String> camera =
                  new EnumMap<CameraProperty, String>(CameraProperty.class);
              if (maker != null) {
  ...
  public Optional<Map<AddressComponent, String>> getLocationTaken() {
    return extract(m -> m.getFirstDirectoryOfType(GpsDirectory.class))
  ...
  protected Map<AddressComponent, String> locate(GeoLocation location) {
    try {
      ...
      Object json =
          cache
              .json(location.getLatitude() + "," + location.getLongitude(), this::getGeocodeRequest)
              .get();

      Map<AddressComponent, String> address =
          new EnumMap<AddressComponent, String>(AddressComponent.class);

      streamJsonObjects(json, "results")
          .limit(1)
          .forEach(
              r -> {
                streamJsonObjects(r, "address_components")
                    .forEach(
                        a -> {
                          String name = getString(a, "long_name");
                          if (name != null) {
                            for (Object type : getArray(a, "types")) {
                              stream(AddressComponent.values())
  ...
  public static final FileFilter SUPPORTED_FILE_TYPES =
      new ExtensionFileFilter(
          "jpg", "jpeg", "png", "webp", "gif", "ico", "bmp", "tif", "tiff", "psd", "pcx", "raw",
          "crw", "cr2", "nef", "orf", "raf", "rw2", "rwl", "srw", "arw", "dng", "x3f", "mov", "mp4",
          "m4v", "3g2", "3gp", "3gp");
}
