...
public class ReleaseInfo {
  ...
  protected String matchLast(Pattern pattern, String[] paragon, CharSequence... sequence) {
    ...
    String lastMatch =
        stream(sequence)
            ...
            .map(s -> matchLastOccurrence(s, pattern))
            .filter(Objects::nonNull)
            .findFirst()
            .orElse(null);
  ...
  public String substringBefore(String item, Pattern... stopwords) {
    for (Pattern it : stopwords) {
      ...
      if (matcher.find()) {
        String substring =
            item.substring(0, matcher.start()); // use substring before the matched stopword
  ...
  public Set<File> getVolumeRoots() {
    if (volumeRoots == null) {
      ...
      if (isMacSandbox()) {
        ...
        // e.g. ignore default Movie folder (user.home and real user home are different in the
        // sandbox environment)
  ...
  public Pattern getLanguageTagPattern(boolean strict) {
    ...
    if (strict) {
      return compile(
          "(?<=[-\\[\\{\\(])"
              + or(quoteAll(getDefaultLanguageMap().keySet()))
              + "(?=[-\\]\\}\\)]|$)",
          CASE_INSENSITIVE);
    }

    // FR
    List<String> allCapsLanguageTags =
        getDefaultLanguageMap().keySet().stream().map(String::toUpperCase).collect(toList());
  ...
  public Pattern getVideoFormatPattern(boolean strict) {
    ...
    return strict
        ? compile("(?<!\\p{Alpha})(" + pattern + ")(?!\\p{Alpha})", CASE_INSENSITIVE)
        : compile(pattern, CASE_INSENSITIVE);
  }

  public Pattern getVideoSourcePattern() {
    return compileWordPattern(
        getProperty("pattern.video.source")); // pattern matching any video source name, like BluRay
  }

  public Pattern getVideoTagPattern() {
    return compileWordPattern(
        getProperty("pattern.video.tags")); // pattern matching any video tag, like Directors Cut
  }

  public Pattern getStereoscopic3DPattern() {
    return compileWordPattern(
        getProperty("pattern.video.s3d")); // pattern matching any 3D flags like 3D.HSBS
  }
  ...
  public Pattern getClutterBracketPattern(boolean strict) {
    ...
    return IntStream.range(0, brackets.length() / 2)
        .map(i -> i * 2)
        .mapToObj(
            i -> {
  ...
  public Pattern getBlacklistPattern() throws Exception {
    return compileWordPattern(
        queryBlacklist.get()); // pattern matching any release group name enclosed in separators
  }

  private Pattern compileWordPattern(String[] patterns) {
    return compile(
        "(?<!\\p{Alnum})" + or(patterns) + "(?!\\p{Alnum})",
        CASE_INSENSITIVE); // use | to join patterns
  }
  ...
  public FileFilter getDiskFolderEntryFilter() {
    if (diskFolderEntryFilter == null) {
      diskFolderEntryFilter =
          new RegexMatchFilter(compile(getProperty("pattern.diskfolder.entry")));
    }
  ...
  public FileFilter getClutterTypeFilter() {
    if (clutterTypeFilter == null) {
      clutterTypeFilter =
          new ClutterFileFilter(
              new RegexFindFilter(compile(getProperty("pattern.clutter.types"), CASE_INSENSITIVE)),
              Long.parseLong(getProperty("number.clutter.maxfilesize")));
    }
  ...
  public FileFilter getClutterFileFilter() {
    if (clutterFileFilter == null) {
      clutterFileFilter =
          new ClutterFileFilter(
              new FileFolderNameFilter(getExcludePattern()),
              Long.parseLong(getProperty("number.clutter.maxfilesize")));
    }
  ...
  public FileFilter getSystemFilesFilter() {
    if (systemFilesFilter == null) {
      systemFilesFilter =
          new RegexMatchFilter(compile(getProperty("pattern.system.files"), CASE_INSENSITIVE));
    }
  ...
  private final Resource<Map<Pattern, String>> seriesMappings =
      resource("url.series-mappings", Cache.ONE_WEEK, Function.identity(), String[]::new)
          .transform(
              lines -> {
                ...
                stream(lines)
                    ...
                    .forEach(
                        v -> {
                          Pattern pattern =
                              compile(
                                  "(?<!\\p{Alnum})(" + v[0] + ")(?!\\p{Alnum})", CASE_INSENSITIVE);
                          map.put(pattern, v[1]);
                        });
  ...
  private final Resource<SearchResult[]> tvdbIndex =
      tsv("url.thetvdb-index", Cache.ONE_WEEK, this::parseSeries, SearchResult[]::new);
  private final Resource<SearchResult[]> anidbIndex =
      tsv("url.anidb-index", Cache.ONE_WEEK, this::parseSeries, SearchResult[]::new);

  private final Resource<Movie[]> movieIndex =
      tsv("url.movie-list", Cache.ONE_MONTH, this::parseMovie, Movie[]::new);
  private final Resource<SubtitleSearchResult[]> osdbIndex =
      tsv("url.osdb-index", Cache.ONE_MONTH, this::parseSubtitle, SubtitleSearchResult[]::new);

  private final SystemProperty<Duration> refreshDuration =
      SystemProperty.of("url.refresh", Duration::parse);
  ...
  private Movie parseMovie(String[] v) {
    ...
    return new Movie(
        name, aliasNames, year, imdbid > 0 ? imdbid : -1, tmdbid > 0 ? tmdbid : -1, null);
  }
  ...
  private SubtitleSearchResult parseSubtitle(String[] v) {
    ...
    return new SubtitleSearchResult(
        ...
        -1,
        Locale.ENGLISH,
        SubtitleSearchResult.Kind.forName(kind),
        score);
  }
  ...
  protected <A> Resource<A[]> tsv(
      String name,
      Duration expirationTime,
      Function<String[], A> parse,
      IntFunction<A[]> generator) {
    return resource(name, expirationTime, s -> parse.apply(TAB.split(s)), generator).memoize();
  }

  protected <A> Resource<A[]> resource(
      String name, Duration expirationTime, Function<String, A> parse, IntFunction<A[]> generator) {
    return () -> {
      Cache cache = Cache.getCache("data", CacheType.Persistent);
      byte[] bytes =
          cache
              .bytes(name, n -> new URL(getProperty(n)), XZInputStream::new)
              .expire(refreshDuration.optional().orElse(expirationTime))
              .get();
  ...
  private static Properties getReleaseInfoProperties() {
    ...
    try (var stream =
        ReleaseInfo.class.getClassLoader().getResourceAsStream("ReleaseInfo.properties")) {
  ...
  public static class FileFolderNameFilter implements FileFilter {
    ...
    public boolean accept(File file) {
      if (file.isFile()) {
        ...
        return namePattern.matcher(getNameWithoutExtension(file.getName())).find()
            || namePattern.matcher(file.getParentFile().getName()).find();
      } else {
  ...
  private String or(Object[] terms) {
    return join(
        stream(terms).sorted(reverseOrder()),
        "|",
        "(",
        ")"); // non-capturing group that matches the longest occurrence
  }
  ...
  public Map<String, Locale> getLanguageMap(Locale... displayLanguages) {
    ...
    for (String code : Locale.getISOLanguages()) {
      ...
      for (Locale language : displayLanguages) {
        // make sure language name is properly normalized so accents and whatever don't break the
    ...
    // missing ISO 639-2 (B/T) locales (see
    // https://github.com/TakahikoKawasaki/nv-i18n/blob/master/src/main/java/com/neovisionaries/i18n/LanguageAlpha3Code.java)
    ...
    languageMap.remove(
        "hi"); // hi => typically used for hearing-impaired subtitles, NOT hindi language
