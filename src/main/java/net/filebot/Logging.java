...
public final class Logging {
  ...
  private static final SystemProperty<Level> debugLevel =
      SystemProperty.of("net.filebot.logging.debug", Level::parse, Level.WARNING);
  private static final SystemProperty<Pattern> anonymizePattern =
      SystemProperty.of("net.filebot.logging.anonymize", Pattern::compile);
  private static final SystemProperty<Boolean> color =
      SystemProperty.of(
          "net.filebot.logging.color", Boolean::parseBoolean, EscapeCode.isSupported());
  private static final SystemProperty<Boolean> timeStamp =
      SystemProperty.of("net.filebot.logging.time", Boolean::parseBoolean, false);
  private static final SystemProperty<DateTimeFormatter> timeStampFormat =
      SystemProperty.of(
          "net.filebot.logging.time.format",
  ...
  public static StreamHandler createSimpleFileHandler(File file, Level level) throws IOException {
    StreamHandler handler =
        new StreamHandler(new FileOutputStream(file, true), new SimpleFormatter());
    handler.setEncoding("UTF-8");
    handler.setLevel(level);
    return handler;
  }

  public static StreamHandler createLogFileHandler(File file, boolean lock, Level level)
      throws IOException {
    ...
    StreamHandler handler =
        new StreamHandler(
            ...
            new ConsoleFormatter(anonymizePattern.get(), false, createTimeStampFormat()));
    handler.setEncoding("UTF-8");
    handler.setLevel(level);
    return handler;
  }
  ...
  public static ConsoleHandler createConsoleHandler(Level level) {
    ...
    handler.setFormatter(
        new ConsoleFormatter(anonymizePattern.get(), color.get(), createTimeStampFormat()));
    return handler;
  }

  private static DateTimeFormatter createTimeStampFormat() {
    return timeStamp.get()
        ? timeStampFormat.get().withLocale(Locale.US).withZone(ZoneId.systemDefault())
        : null;
  }
  ...
  public static String getMessage(Object m, Throwable t) {
    ...
    if (t.getCause() != null
        && (t instanceof RuntimeException || t instanceof ExecutionException)) {
  ...
  public static class ConsoleFormatter extends Formatter {
    ...
    public ConsoleFormatter(
        Pattern anonymize, boolean colorize, DateTimeFormatter dateTimeFormatter) {
    ...
    public EscapeCode getColor(int level) {
      if (colorize) {
        if (level < Level.FINE.intValue()) return EscapeCode.LIME_GREEN;
        if (level < Level.INFO.intValue()) return EscapeCode.ROYAL_BLUE;
        if (level < Level.WARNING.intValue()) return EscapeCode.NONE; // LOG INFO => NO COLOR
        if (level < Level.SEVERE.intValue()) return EscapeCode.ORANGE_RED;
  ...
  public static class ConsoleHandler extends Handler {
    ...
    public void publish(LogRecord record) {
      ...
      PrintStream out =
          record.getLevel().intValue() < Level.WARNING.intValue() ? System.out : System.err;
