...
public class PresetEditor extends JDialog {
  ...
  public PresetEditor(Window owner) {
    ...
    inheritRadio =
        new JRadioButton("<html><nobr>Use <b>Original Files</b> selection</nobr></html>");
    selectRadio = new JRadioButton("<html><nobr>Do <b>Select</b> files</nobr></html>");
    pathInput = new JTextField(40);

    filterEditor =
        new FormatExpressionTextArea(new RSyntaxDocument(RSyntaxDocument.SYNTAX_STYLE_GROOVY));
  ...
  public void updateComponentStates() {
    ...
    languageCombo.setEnabled(
        ds instanceof EpisodeListProvider || ds instanceof MovieIdentificationService);
    matchModeCombo.setEnabled(
        ds instanceof EpisodeListProvider || ds instanceof MovieIdentificationService);
  }
  ...
  public void setPreset(Preset p) {
    ...
    filterEditor.setText(
        p.getIncludeFilter() == null
            ? ""
            : p.getIncludeFilter().getExpressionFilter().getExpression());
    formatEditor.setText(p.getFormat() == null ? "" : p.getFormat().getExpression());
    providerCombo.setSelectedItem(
        p.getDatasource() == null ? WebServices.TheTVDB : p.getDatasource());
    sortOrderCombo.setSelectedItem(p.getSortOrder() == null ? SortOrder.Airdate : p.getSortOrder());
    matchModeCombo.setSelectedItem(
        p.getMatchMode() == null ? RenamePanel.MATCH_MODE_OPPORTUNISTIC : p.getMatchMode());
    actionCombo.setSelectedItem(
        p.getRenameAction() == null ? StandardRenameAction.MOVE : p.getRenameAction());

    // ugly hack, since Language objects only do object equality
    if (p.getLanguage() != null
        && !p.getLanguage()
            .getCode()
            .equals(((Language) languageCombo.getSelectedItem()).getCode())) {
  ...
  public Preset getPreset() throws Exception {
    ...
    ExpressionFilter includes =
        inheritRadio.isSelected() ? null : new ExpressionFilter(filterEditor.getText());
    ExpressionFileFormat format =
        formatEditor.getText().trim().isEmpty()
            ? null
            : new ExpressionFileFormat(formatEditor.getText());
    Datasource database = ((Datasource) providerCombo.getSelectedItem());
    SortOrder sortOrder =
        sortOrderCombo.isEnabled() ? ((SortOrder) sortOrderCombo.getSelectedItem()) : null;
    String matchMode =
        matchModeCombo.isEnabled() ? (String) matchModeCombo.getSelectedItem() : null;
    Language language =
        languageCombo.isEnabled() ? ((Language) languageCombo.getSelectedItem()) : null;
    StandardRenameAction action =
        actionCombo.isEnabled() ? (StandardRenameAction) actionCombo.getSelectedItem() : null;

    return new Preset(
        name, path, includes, format, database, sortOrder, matchMode, language, action);
  }
        ...
        (list, value, index, isSelected, cellHasFocus) -> {
          ...
          if (value instanceof Datasource) {
            ...
            label.setToolTipText(
                String.format("%s Mode: %s", Mode.getMode(provider), provider.getName()));
          }
  ...
  private final Action editFormatExpression =
      newAction(
          ...
          evt -> {
            ...
            if (mode == Mode.File) {
              List<File> files =
                  UserFiles.showLoadDialogSelectFiles(
                      false,
                      false,
                      null,
                      new ExtensionFileFilter(ExtensionFileFilter.WILDCARD),
                      "Select Sample File",
                      evt);
              if (files.isEmpty()) {
                return;
            ...
            FormatDialog dialog =
                new FormatDialog(
                    ...
                    mode,
                    new MediaBindingBean(sample, file, singletonMap(file, sample)),
                    false);
  ...
  private final Action listFiles =
      newAction(
          "List Files",
          ResourceManager.getIcon("action.search"),
          evt -> {
            withWaitCursor(
                evt.getSource(),
                () -> {
                  try {
                    ...
                    if (isMacSandbox()) {
                      if (!MacAppUtilities.askUnlockFolders(
                          getWindow(evt.getSource()), singleton(preset.getInputFolder()))) {
                        return;
                      }
                    }
  ...
  private static final String FILE_FILTER_TOOLTIP =
      "<html>File Selector Expression<br><hr noshade>e.g.<br>• fn =~ /alias/<br>• ext =~ /mp4/<br>• minutes &gt; 100<br>• age &lt; 7<br>• file.isEpisode()<br>• …<br></html>";
}
