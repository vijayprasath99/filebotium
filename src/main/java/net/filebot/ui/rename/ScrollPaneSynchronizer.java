...
class ScrollPaneSynchronizer {
  ...
  public ScrollPaneSynchronizer(RenameList... components) {
    ...
    BoundedRangeModel horizontalScrollBarModel =
        components[0].getListScrollPane().getHorizontalScrollBar().getModel();
    BoundedRangeModel verticalScrollBarModel =
        components[0].getListScrollPane().getVerticalScrollBar().getModel();

    // recalculate common size on change
    ListDataListener resizeListener =
  ...
  public void updatePreferredSize() {
    ...
    for (RenameList component : components) {
      ...
      // update maximum size
      if (preferred.width > max.width) max.width = preferred.width;
      if (preferred.height > max.height) max.height = preferred.height;
    }
