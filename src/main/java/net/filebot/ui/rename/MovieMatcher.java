...
class MovieMatcher implements AutoCompleteMatcher {
  ...
  private Map<String, Movie> selectionMemory =
      new TreeMap<String, Movie>(getLenientCollator(Locale.ENGLISH));
  private Map<String, String> inputMemory =
      new TreeMap<String, String>(getLenientCollator(Locale.ENGLISH));
  ...
  public List<Match<File, ?>> match(
    ...
    // ignore sample files
    List<File> fileset =
        autodetect ? filter(files, not(getClutterFileFilter())) : new ArrayList<File>(files);
    ...
    try {
      List<Future<Map<File, List<Movie>>>> tasks =
          movieMatchFiles.stream()
              .filter(f -> movieByFile.get(f) == null)
              .map(
                  f -> {
                    return workerThreadPool.submit(
                        () -> {
                          ...
                          // ignore files that cannot yield any acceptable matches (e.g. movie files
                          // without year in strict mode)
                          if (options == null) {
    ...
    Map<Movie, Set<File>> filesByMovie =
        movieByFile.entrySet().stream()
            .collect(
                groupingBy(
                    Entry::getValue,
                    LinkedHashMap::new,
  ...
  protected Movie grabMovieName(
      File movieFile,
      Collection<Movie> options,
      boolean strict,
      Locale locale,
      boolean autodetect,
      Component parent)
    ...
    synchronized (selectionMemory) {
      if (!strict
          && (!autodetect || options.isEmpty())
          && !(autodetect && autoSelectionMode.size() > 0)) {
        String suggestion =
            options.isEmpty()
                ? stripReleaseInfo(getName(movieFile))
                : options.iterator().next().getName();
          ...
          input =
              showInputDialog(
                  getQueryInputMessage(
                      "Please identify the following files:", "Enter movie name:", movieFile),
                  suggestion != null && suggestion.length() > 0 ? suggestion : getName(movieFile),
                  movieFolder == null
                      ? movieFile.getName()
                      : String.join(" / ", movieFolder.getName(), movieFile.getName()),
                  parent);
  ...
  protected Movie selectMovie(
      File movieFile, boolean strict, String userQuery, Collection<Movie> options, Component parent)
      throws Exception {
    ...
    File movieFolder = guessMovieFolder(movieFile);
    String folderQuery =
        (userQuery != null || movieFolder == null)
            ? ""
            : checkedStripReleaseInfo(movieFolder, strict);
    ...
    for (Movie movie : options) {
      ...
      if (fileQuery.toLowerCase().startsWith(movieIdentifier)
          || folderQuery.toLowerCase().startsWith(movieIdentifier)) {
        return movie;
      }
    }
    ...
    for (Movie result : options) {
      ...
      for (String query : new String[] {fileQuery, folderQuery}) {
        for (String name :
            strict ? result.getEffectiveNames() : result.getEffectiveNamesWithoutYear()) {
    ...
    // if we haven't confirmed a match at this point then the file is probably badly named and
    // should be ignored
    if (strict) {
      return null;
    }
    ...
    Callable<Movie> showSelectDialog =
        () -> {
          String query =
              fileQuery.length() >= 2 || folderQuery.length() <= 2 ? fileQuery : folderQuery;
          JLabel header =
              new JLabel(
                  getQueryInputMessage(
                      "Failed to identify some of the following files:", null, movieFile));
          header.setBorder(
              createCompoundBorder(createTitledBorder(""), createEmptyBorder(3, 3, 3, 3)));

          // multiple results have been found, user must select one
          SelectDialog<Movie> selectDialog =
              new SelectDialog<Movie>(parent, options, true, false, header);
          ...
          if (selectDialog.getAutoRepeatCheckBox().isSelected()
              || selectDialog.getSelectedAction() == null) {
            autoSelectionMode.add(
                selectDialog.getSelectedValue() == null ? AutoSelection.Skip : AutoSelection.First);
          }
