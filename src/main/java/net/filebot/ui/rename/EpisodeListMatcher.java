...
class EpisodeListMatcher implements AutoCompleteMatcher {
  ...
  private Map<String, SearchResult> selectionMemory =
      new TreeMap<String, SearchResult>(getLenientCollator(Locale.ENGLISH));
  private Map<String, List<String>> inputMemory =
      new TreeMap<String, List<String>>(getLenientCollator(Locale.ENGLISH));
  ...
  public TypedCache<SearchResult> getPersistentSelectionMemory() {
    return Cache.getCache("selection_" + provider.getName(), CacheType.Persistent)
  ...
  public List<Match<File, ?>> match(
    ...
    // ignore sample files
    List<File> fileset =
        autodetection ? filter(files, not(getClutterFileFilter())) : new ArrayList<File>(files);
    ...
    try {
      ...
      if (strict) {
        // in strict mode simply process file-by-file (ignoring all files that don't contain clear
        // SxE patterns)
        mediaFiles.stream()
            .filter(f -> isEpisode(f, false))
            .map(
                f -> {
                  return workerThreadPool.submit(
                      () -> {
                        return matchEpisodeSet(
                            singletonList(f),
                            detectSeriesNames(singleton(f), anime, locale),
                            sortOrder,
                            strict,
                            locale,
                            autodetection,
        ...
        // in non-strict mode use the complicated (more powerful but also more error prone)
        // match-batch-by-batch logic
        mapSeriesNamesByFiles(mediaFiles, locale, anime)
            .forEach(
                (f, n) -> {
                  // 1. handle series name batch set all at once -> only 1 batch set
                  // 2. files don't seem to belong to any series -> handle folder per folder ->
                  // multiple batch sets
                  Collection<List<File>> batches =
                      n != null && n.size() > 0
                          ? singleton(new ArrayList<File>(f))
                          : mapByFolder(f).values();
                      ...
                      .map(
                          b -> {
                            return workerThreadPool.submit(
                                () -> {
                                  return matchEpisodeSet(
                                      b, n, sortOrder, strict, locale, autodetection, parent);
                                });
                          })
                      .forEach(tasks::add);
                });
      ...
      for (Future<List<Match<File, ?>>> future : tasks) {
        ...
        for (Match<File, ?> it : future.get()) {
          matches.add(
              new Match<File, Episode>(it.getValue(), ((Episode) it.getCandidate()).clone()));
        }
      }
    } finally {
  ...
  public List<Match<File, ?>> matchEpisodeSet(
      List<File> files,
    ...
    if (episodes.isEmpty() && !strict) {
      ...
      String suggestion =
          detectedSeriesNames.size() > 0
              ? join(detectedSeriesNames, "; ")
              : normalizePunctuation(getName(files.get(0)));

      synchronized (inputMemory) {
        List<String> input = inputMemory.get(suggestion);
        if (input == null || suggestion == null || suggestion.isEmpty()) {
          input =
              showMultiValueInputDialog(
                  getQueryInputMessage(
                      "Please identify the following files:", "Enter series name:", files),
                  suggestion,
                  provider.getName(),
                  parent);
  ...
  protected Set<Episode> fetchEpisodeSet(
      List<File> files,
    ...
    List<Future<List<Episode>>> tasks =
        querySet.stream()
            .map(
                q -> {
                      ...
                      () -> {
                        ...
                        if (options.size() > 0) {
                          SearchResult selectedSearchResult =
                              selectSearchResult(files, q, options, autodetection, parent);
  ...
  protected SearchResult selectSearchResult(
      List<File> files,
      String query,
    ...
    Callable<SearchResult> showSelectDialog =
        () -> {
          JLabel header =
              new JLabel(
                  getQueryInputMessage(
                      ...
                      null,
                      getFilesForQuery(files, query)));
          header.setBorder(
              createCompoundBorder(createTitledBorder(""), createEmptyBorder(3, 3, 3, 3)));

          // multiple results have been found, user must select one
          SelectDialog<SearchResult> selectDialog =
              new SelectDialog<SearchResult>(
                  parent, options, true, false, header.getText().isEmpty() ? null : header);
          selectDialog.setTitle(provider.getName());
          selectDialog
              .getMessageLabel()
              .setText("<html>Select best match for \"<b>" + escapeHTML(query) + "</b>\":</html>");
          ...
          if (selectDialog.getAutoRepeatCheckBox().isSelected()
              && selectDialog.getSelectedValue() != null) {
  ...
  protected Collection<File> getFilesForQuery(Collection<File> files, String query) {
    Pattern pattern =
        Pattern.compile(
            query.isEmpty() ? ".+" : normalizePunctuation(query).replaceAll("\\W+", ".+"),
  ...
  protected String getQueryInputMessage(String header, String message, Collection<File> files)
      throws Exception {
    List<File> selection =
        files.stream()
            .sorted(comparing(File::length).reversed())
            .limit(4)
            .sorted(HUMAN_NAME_ORDER)
            .collect(toList());
  ...
  public List<Match<File, ?>> justFetchEpisodeList(
      SortOrder sortOrder, Locale locale, Component parent) throws Exception {
    // require user input
    List<String> input =
        showMultiValueInputDialog("Enter series name:", "", "Fetch Episode List", parent);

    List<Match<File, ?>> matches = new ArrayList<Match<File, ?>>();
    if (input.size() > 0) {
      Collection<Episode> episodes =
          fetchEpisodeSet(emptyList(), input, sortOrder, locale, false, parent);
