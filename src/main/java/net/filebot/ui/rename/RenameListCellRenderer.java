...
class RenameListCellRenderer extends DefaultFancyListCellRenderer {
  ...
  @Override
  public void configureListCellRendererComponent(
    ...
    if (renameModel.preserveExtension()
        && index < renameModel.size()
        && renameModel.getMatch(index).getCandidate() != null) {
    ...
    if (value instanceof File) {
      ...
      if (renameModel.preserveExtension()) {
        ...
        setText(
            isSelected || !renameModel.hasComplement(index)
                ? formatPath(file)
                : colorizePath(file, true));
      }
      ...
      float matchProbablity =
          renameModel.hasComplement(index) ? getMatchProbablity(formattedFuture.getMatch()) : 1;
      ...
      if (formattedFuture.isDone() && !formattedFuture.isCancelled()) {
        if (!renameModel.preserveExtension() && renameModel.hasComplement(index)) {
          ...
          setText(
              isSelected || matchProbablity < 1 || !renameModel.hasComplement(index)
                  ? formatPath(path)
                  : colorizePath(path, !renameModel.preserveExtension()));
        }
      } else {
      ...
      if (renameModel.hasComplement(index)) {
        ...
        setBackground(
            derive(
                warningGradientBeginColor,
                (1 - matchProbablity) * 0.5f)); // alpha indicates match probability
        ...
        if (pathFuture.isDone() && !pathFuture.isCancelled()) {
          ...
          File to =
              resolveAbsolutePath(
                  from.getParentFile(),
                  pathFuture.toString(),
                  renameModel.preserveExtension() ? getExtension(from) : null);
          if (equalsCaseSensitive(from, to)) {
            ...
            setIcon(
                ResourceManager.getIcon(
                    "dialog.cancel")); // take into account that on Windows equals/exists is
            // case-insensitive which we have to work around
          }
        }
      }
    }
    ...
    File f =
        new File(
            extension == null || extension.isEmpty()
                ? path
                : String.format("%s.%s", path, extension));
    if (!f.isAbsolute()) {
  ...
  protected float getMatchProbablity(Match<Object, File> match) {
    ...
    // check match probability by running a few metrics, such as checking if episode numbers / file
    // size / etc match
    ...
    if (f != 0) {
      return (Math.max(
          f, 0)); // normalize -1..1 and boost by 0.25 (because file <-> file matches are not
      // necessarily about Episodes)
    }
  ...
  private static class TypeRenderer extends DefaultListCellRenderer {
    ...
    protected void paintComponent(Graphics g) {
      ...
      g2d.setPaint(
          GradientStyle.TOP_TO_BOTTOM.getGradientPaint(
              shape, gradientBeginColor, gradientEndColor));
      g2d.fill(shape);
      ...
      g2d.drawString(
          getText(),
          (float) (shape.getCenterX() - textBounds.getX() - (textBounds.getWidth() / 2f)),
          (float) (shape.getCenterY() - textBounds.getY() - (textBounds.getHeight() / 2)));
    }
