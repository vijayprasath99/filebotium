...
class BindingDialog extends JDialog {
  ...
  public BindingDialog(Window owner, String title, Format infoObjectFormat, boolean editable) {
    ...
    root.setLayout(
        new MigLayout(
            "nogrid, novisualpadding, fill, insets dialog",
            "",
            "[pref!]paragraph[pref!]2px[grow,fill]paragraph[pref!]"));
    ...
    inputPanel.add(
        createImageButton(selectFileAction), "gap rel, w 28px!, h 28px!, wrap paragraph");
    ...
    if (editable) {
      root.add(
          newButton(
              "Use Bindings", ResourceManager.getIcon("dialog.continue"), evt -> finish(true)),
          "tag apply");
      root.add(
          newButton("Cancel", ResourceManager.getIcon("dialog.cancel"), evt -> finish(false)),
          "tag cancel");
    } else {
      root.add(
          newButton("Close", ResourceManager.getIcon("dialog.continue"), e -> finish(false)),
          "tag ok");
    }
  ...
  private void updatePreviewModel() {
    ...
    String[] expressions =
        COMMA.split(ResourceBundle.getBundle(getClass().getName()).getString("expressions"));
    bindingModel.setModel(asList(expressions), sample);
  }

  protected final Action mediaInfoAction =
      new AbstractAction("Open MediaInfo", ResourceManager.getIcon("action.properties")) {
        ...
        public void actionPerformed(ActionEvent evt) {
          ...
          RowFilter<Object, Object> excludeRowFilter =
              RowFilter.notFilter(RowFilter.regexFilter(bundle.getString("parameter.exclude")));
          ...
          JDialog dialog =
              new JDialog(getWindow(evt.getSource()), "MediaInfo", ModalityType.DOCUMENT_MODAL);
  ...
  protected final Action selectFileAction =
      new AbstractAction("Select Media File", ResourceManager.getIcon("action.load")) {

        @Override
        public void actionPerformed(ActionEvent evt) {
          ExtensionFileFilter mediaFiles =
              ExtensionFileFilter.union(VIDEO_FILES, AUDIO_FILES, SUBTITLE_FILES, IMAGE_FILES);
          List<File> selection =
              showLoadDialogSelectFiles(
                  false, false, getMediaFile(), mediaFiles, (String) getValue(NAME), evt);
  ...
  private static class BindingTableModel extends AbstractTableModel {
    ...
    private final ExecutorService executor =
        Executors.newFixedThreadPool(1, new DefaultThreadFactory("Evaluator", Thread.MIN_PRIORITY));
    ...
    public void setModel(Collection<String> expressions, Object bindingBean) {
      ...
      for (String expression : expressions) {
        Evaluator evaluator =
            new Evaluator(expression, bindingBean) {
