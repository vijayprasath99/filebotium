...
public class FormatExpressionTextArea extends RSyntaxTextArea {
  ...
  public FormatExpressionTextArea() {
    this(
        new RSyntaxDocument(
            new FormatExpressionTokenMakerFactory(),
            FormatExpressionTokenMakerFactory.SYNTAX_STYLE_GROOVY_FORMAT_EXPRESSION));
  }
  ...
  public FormatExpressionTextArea(RSyntaxDocument syntaxDocument) {
    ...
    // dynamically resize the code editor depending on how many lines the current format expression
    // has
    getDocument()
        .addDocumentListener(
