...
public class TextColorizer {
  ...
  public TextColorizer(
      String before, String after, Color pathRainbowBeginColor, Color pathRainbowEndColor) {
    this.before = before;
    this.after = after;
  ...
  public StringBuilder colorizePath(StringBuilder html, File file, boolean hasExtension) {
    ...
    for (int i = 0; i < path.size() - 1; i++) {
      ...
      html.append(
          String.format(
              "<span style='color:rgb(%1$d, %2$d, %3$d)'>%4$s</span><span style='color:rgb(%1$d, %2$d, %3$d)'>/</span>",
              c.getRed(), c.getGreen(), c.getBlue(), escapeHTML(getFolderName(path.get(i)))));
    }
    ...
    if (hasExtension) {
      ...
      if (extension != null) {
        html.append(
            String.format(
                "<span style='color:#607080'>.%s</span>",
                escapeHTML(extension))); // highlight extension
      }
    } else {
