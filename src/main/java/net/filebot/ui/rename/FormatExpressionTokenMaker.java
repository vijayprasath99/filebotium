...
public class FormatExpressionTokenMaker extends TokenMakerBase {
  ...
  private static final GroovyExpressionTokenMaker groovyTokenMaker =
      new GroovyExpressionTokenMaker();
    ...
    for (int i = 0; i < segment.length(); i++) {
      switch (segment.charAt(i)) {
        case '{':
          if (level <= 0) {
            if (start != end) {
              addToken(
                  segment,
                  segment.getBeginIndex() + start,
                  segment.getBeginIndex() + end - 1,
                  Token.MARKUP_CDATA,
                  startOffset + start);
            }
            ...
            addToken(
                segment,
                segment.getBeginIndex() + i,
                segment.getBeginIndex() + i,
                Token.MARKUP_CDATA_DELIMITER,
                startOffset + i);
            start = end = i + 1;
            level = 0;
          } else {
        ...
        case '}':
          if (level == 1) {
            if (start != end) {
              Segment groovySegment =
                  new Segment(segment.array, segment.getBeginIndex() + start, end - start);
              addToken(
                  groovyTokenMaker.getTokenList(
                      groovySegment, initialTokenType, startOffset + start));
            }
            addToken(
                segment,
                segment.getBeginIndex() + i,
      ...
      case LANGUAGE_GROOVY:
        ...
        if (start <= end) {
          Segment groovySegment =
              new Segment(segment.array, segment.getBeginIndex() + start, end - start);
          addToken(
              groovyTokenMaker.getTokenList(groovySegment, initialTokenType, startOffset + start));
        }
        addLevelToken(level);
        break;
      default:
  ...
  public int getClosestStandardTokenTypeForInternalType(int type) {
    return type < 0
        ? TokenTypes.NULL
        : groovyTokenMaker.getClosestStandardTokenTypeForInternalType(type);
  }
