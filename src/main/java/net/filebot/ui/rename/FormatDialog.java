...
public class FormatDialog extends JDialog {
  ...
  private static final PreferencesEntry<String> persistentSampleFile =
      Settings.forPackage(FormatDialog.class).entry("format.sample.file");
  ...
  public enum Mode {
    ...
    public Iterable<String> getSampleExpressions() {
      ...
      for (String key : bundle.keySet()) {
        if (key.startsWith(prefix)) examples.put(key, bundle.getString(key));
      }
    ...
    public static Mode getMode(Datasource datasource) {
      if (datasource instanceof MovieIdentificationService) return Mode.Movie;
      if (datasource instanceof EpisodeListProvider) return Mode.Episode;
      if (datasource instanceof MusicIdentificationService) return Mode.Music;
  ...
  public FormatDialog(Window owner, Mode initMode, MediaBindingBean bindings, boolean locked) {
    ...
    content.add(
        createImageButton(changeSampleAction),
        "sg action, w 25!, h 19!, pos n editor.y2+2 editor.x2 n");
    content.add(
        createImageButton(selectFolderAction),
        "sg action, w 25!, h 19!, pos n editor.y2+2 editor.x2-(27*1) n");
    content.add(
        createImageButton(showRecentAction),
        "sg action, w 25!, h 19!, pos n editor.y2+2 editor.x2-(27*2) n");
    ...
    addPropertyChangeListener(
        ...
        evt -> {
          if (isMacSandbox()) {
            if (sample != null
                && sample.getFileObject() != null
                && sample.getFileObject().exists()) {
              MacAppUtilities.askUnlockFolders(
                  getWindow(evt.getSource()), singleton(sample.getFileObject()));
            }
          }
          checkFormatInBackground();
        });
    ...
    if (bindings == null) {
      ...
      bindings =
          new MediaBindingBean(bindings.getInfoObject(), restoreSample(initMode).getFileObject());
    }

    // install show / hide help keyboard shortcut
    installAction(
        this.getRootPane(),
        getKeyStroke(VK_F1, 0),
        newAction("Help", evt -> help.setVisible(!help.isVisible())));
  ...
  public void setState(Mode mode, MediaBindingBean bindings, boolean locked) {
    ...
    if (locked) {
      ...
      title.setText(
          String.format(
              "%s ⇔ %s",
              mode,
              bindings.getInfoObject(),
              bindings.getFileObject() == null ? null : bindings.getFileObject().getName()));
    } else {
      this.setTitle(String.format("%s Format", mode));
    ...
    setFormatCode(
        mode.persistentFormatHistory().isEmpty()
            ? mode.getDefaultFormatExpression()
            : mode.persistentFormatHistory().get(0));
  ...
  private JComponent createSyntaxPanel(Mode mode) {
    JPanel panel =
        new JPanel(new MigLayout("fill, nogrid, novisualpadding", "[pref]", "[fill, min]"));
    ...
    panel.add(
        new LinkButton(
            newAction(
                ResourceBundle.getBundle(FormatDialog.class.getName())
                    .getString(mode.key() + ".syntax"),
                evt -> {
                  openURI(
                      ResourceBundle.getBundle(FormatDialog.class.getName()).getString("help.url"));
                })),
        "h min!");
  ...
  private ExecutorService createExecutor() {
    ...
    ThreadPoolExecutor executor =
        new ThreadPoolExecutor(
            ...
            new DefaultThreadFactory("PreviewFormatter"));
  ...
  private void checkFormatInBackground() {
    try {
              ...
              try {
                ...
                if (suppressed != null) {
                  ...
                  if (bindingIssue != null
                      && getMessage(bindingIssue)
                          .contains(MediaBindingBean.EXCEPTION_SAMPLE_FILE_NOT_SET)) {
                  ...
                  } else if (suppressed.getCause() != null
                      && suppressed.getCause().getClass() == Exception.class) {
      ...
      // cancel old worker, after new worker has been created, because done() might be called from
      // within cancel()
  ...
  protected final Action changeSampleAction =
      newAction(
          "Change Sample",
          ResourceManager.getIcon("action.variables"),
          evt -> {
            BindingDialog dialog =
                new BindingDialog(
                    getWindow(evt.getSource()),
                    String.format("%s Bindings", mode),
                    mode.getFormat(),
                    !locked);
            ...
            if (dialog.submit()) {
              ...
              try {
                ...
                persistentSampleFile.setValue(
                    file == null ? "" : sample.getFileObject().getAbsolutePath());
              } catch (Exception e) {
  ...
  protected final Action selectFolderAction =
      newAction(
          "Change Folder",
          ResourceManager.getIcon("action.load"),
          evt -> {
              ...
              if (templatePath.isAbsolute()) {
                ...
                for (File next : listPath(templatePath)) {
                  if (existingPath != null && !next.exists()) {
                    ...
                    relativeFormat =
                        relativeFormat.substring(
                            existingPath.getPath().length() + 1); // account for file separator
                    break;
                  }
                  existingPath = next;
                }
            ...
            File selectedFolder =
                UserFiles.showOpenDialogSelectFolder(absoluteFolder, "Select Folder", evt);
            if (selectedFolder != null) {
              editor.setText(
                  normalizePathSeparators(selectedFolder.getAbsolutePath()) + "/" + relativeFormat);
            }
          });
  ...
  protected final Action showRecentAction =
      newAction(
          "Change Format",
          ResourceManager.getIcon("action.menu"),
          evt -> {
  ...
  protected final Action switchEditModeAction =
      newAction(
          "Switch Mode",
          ResourceManager.getIcon("dialog.switch"),
          evt -> {
            Mode next = mode.next();
  ...
  protected final Action approveFormatAction =
      newAction(
          "Use Format",
          ResourceManager.getIcon("dialog.continue"),
          evt -> {
            try {
              // check syntax
              ...
              if (format.getExpression().length() > Preferences.MAX_VALUE_LENGTH) {
                throw new ScriptException(
                    "Expression is limited to " + Preferences.MAX_VALUE_LENGTH + " characters");
              }
              ...
              mode.persistentFormatHistory()
                  .set(recent.stream().filter(Objects::nonNull).limit(8).collect(toList()));
