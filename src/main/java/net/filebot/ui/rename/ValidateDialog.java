...
class ValidateDialog extends JDialog {
  ...
  private final Action validateAction =
      new AbstractAction("Validate", ResourceManager.getIcon("dialog.continue")) {
  ...
  private final Action continueAction =
      newAction(
          "Continue", ResourceManager.getIcon("dialog.continue.invalid"), evt -> finish(false));

  private final Action cancelAction =
      newAction("Cancel", ResourceManager.getIcon("dialog.cancel"), evt -> finish(true));
  ...
  private static class IllegalCharactersListCellRenderer extends HighlightListCellRenderer {
    ...
    public IllegalCharactersListCellRenderer() {
      super(
          ILLEGAL_CHARACTERS,
          new CharacterHighlightPainter(new Color(0xFF4200), new Color(0xFF1200)),
          4);
    }
    ...
    protected void updateHighlighter() {
      ...
      // highlight path components separately to ignore "illegal characters" that are either path
      // separators or part of the drive letter (e.g. ':' in 'E:')
      for (File element : listPath(file)) {
        ...
        while (matcher.find()) {
          try {
            textComponent
                .getHighlighter()
                .addHighlight(matcher.start(0), matcher.end(0), highlightPainter);
