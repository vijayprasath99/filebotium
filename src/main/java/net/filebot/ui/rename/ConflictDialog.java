...
class ConflictDialog extends JDialog {
  ...
  public ConflictDialog(Window owner, List<Conflict> conflicts) {
    ...
    c.add(
        newButton("Continue", ResourceManager.getIcon("dialog.continue"), this::ignore), "tag ok");

    JButton b =
        newButton("Override", ResourceManager.getIcon("dialog.continue.invalid"), this::override);
    ...
    installAction(
        c, KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), newAction("Cancel", this::cancel));
            ...
            .map(
                c -> {
                  ...
                  try {
                    ...
                    return new Conflict(
                        c.source, c.destination, singletonList(e.getMessage()), false);
                  }
  ...
  private static class ConflictTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int row, int column) {
      ...
      switch (column) {
        ...
        case 1:
          return conflict.issues.isEmpty()
              ? "OK"
              : conflict.issues.stream().map(Objects::toString).collect(joining(" • "));
        case 2:
    ...
    public void mouseClicked(MouseEvent evt) {
      if (evt.getClickCount() == 2) {
        ...
        if (row >= 0) {
          ...
          List<File> files =
              Stream.of(c.source, c.destination).filter(File::exists).distinct().collect(toList());
