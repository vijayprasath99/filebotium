...
class RenameList<E> extends FileBotList<E> {
  ...
  public RenameList(EventList<E> model) {
    ...
    buttonPanel =
        new JPanel(new MigLayout("insets 1.2mm, nogrid, novisualpadding, fill", "align center"));
  ...
  private JButton createLoadButton() {
    ...
    actionPopup.add(
        newAction(
            "Select Folders",
            ResourceManager.getIcon("tree.closed"),
            evt -> {
              loadAction.actionPerformed(
                  new ActionEvent(evt.getSource(), evt.getID(), evt.getActionCommand(), 0));
            }));

    actionPopup.add(
        newAction(
            "Select Files",
            ResourceManager.getIcon("file.generic"),
            evt -> {
              loadAction.actionPerformed(
                  new ActionEvent(
                      evt.getSource(),
  ...
  private final Action upAction =
      newAction(
          "Align Up",
          ResourceManager.getIcon("action.up"),
          evt -> {
  ...
  private final Action downAction =
      newAction(
          "Align Down",
          ResourceManager.getIcon("action.down"),
          evt -> {
            int index = getListComponent().getSelectedIndex();
  ...
  private final MouseAdapter dndReorderMouseAdapter =
      new MouseAdapter() {
