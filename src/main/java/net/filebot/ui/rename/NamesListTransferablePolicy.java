...
class NamesListTransferablePolicy extends FileTransferablePolicy {
  ...
  public boolean accept(Transferable tr) throws Exception {
    return hasFileListFlavor(tr)
        || tr.isDataFlavorSupported(stringFlavor)
        || tr.isDataFlavorSupported(episodeArrayFlavor);
  }
  ...
  protected void load(String string) {
    List<String> values =
        NEWLINE
            ...
            .map(String::trim)
            .filter(s -> s.length() > 0)
            .map(s -> normalizePathSeparators(s))
            .collect(toList());
    model.addAll(values);
  }
  ...
  protected void loadListFiles(List<File> files, List<Object> values) throws IOException {
    for (File file : files) {
      // don't use new Scanner(File) because of BUG 6368019
      // (http://bugs.sun.com/view_bug.do?bug_id=6368019)
  ...
  public List<String> getFileFilterExtensions() {
    return Stream.of(
            VIDEO_FILES, SUBTITLE_FILES, AUDIO_FILES, LIST_FILES, TORRENT_FILES, VERIFICATION_FILES)
        .map(ExtensionFileFilter::extensions)
        .flatMap(Stream::of)
        .collect(toList());
  }
}
