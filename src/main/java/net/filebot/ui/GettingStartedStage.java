...
public class GettingStartedStage {
  ...
  public GettingStartedStage(Stage stage) {
    ...
    stage.setScene(
        new Scene(webview, webview.getPrefWidth(), webview.getPrefHeight(), Color.BLACK));

    // make sure that we can read the user locale in JS
    webview
        .getEngine()
        .executeScript(String.format("navigator.locale = '%s'", Locale.getDefault()));
  }
  ...
  protected WebEngine onPopup(WebView webview) {
    ...
    Object uri =
        webview.getEngine().executeScript("$('.galleria').data('galleria').getData().link");
