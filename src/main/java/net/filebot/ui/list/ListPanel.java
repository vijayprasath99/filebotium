...
public class ListPanel extends JComponent {
  ...
  public static final String DEFAULT_EPISODE_FORMAT =
      "{n} - {s00e00} - [{absolute}] - [{airdate}] - {t}";

  private FormatExpressionTextArea editor =
      new FormatExpressionTextArea(new RSyntaxDocument(SyntaxConstants.SYNTAX_STYLE_GROOVY));
  ...
  public ListPanel() {
    ...
    list.getListComponent()
        .getModel()
        .addListDataListener(new PrototypeCellValueUpdater(list.getListComponent(), ""));

    list.getRemoveAction().setEnabled(true);
    list.setTransferablePolicy(
        new FileListTransferablePolicy(
            list::setTitle, this::setFormatTemplate, this::createItemSequence));

    FileBotListExportHandler<ListItem> exportHandler =
        new FileBotListExportHandler<ListItem>(
            list, (item, out) -> out.println(item.getFormattedValue()));
    list.setExportHandler(exportHandler);
    list.getTransferHandler().setClipboardHandler(exportHandler);

    // XXX The user interface of your app is not consistent with the macOS Human Interface
    // Guidelines. Specifically: We found that menu items are not visible, except by right-clicking
    // (see screenshot). See the "WYSIWYG (What You See Is What You Get)," "Give Users
    // Alternate Ways to Accomplish Tasks," and "Designing Contextual Menus" sections of the Human
    ...
    if (!isMacSandbox()) {
        ...
        menu.add(
            newAction(
                ...
                evt -> {
                  ...
                  invokeLater(
                      200, () -> SwingEventBus.getInstance().post(new StringSelection(text)));
                }));
      }
      popup.add(menu);
      popup.addSeparator();
      popup.add(
          newAction(
              "Copy",
              ResourceManager.getIcon("rename.action.copy"),
              evt -> {
                list.getTransferHandler()
                    .getClipboardHandler()
                    .exportToClipboard(
                        this,
    ...
    list.getListComponent()
        .setCellRenderer(
            new DefaultFancyListCellRenderer() {

              @Override
              protected void configureListCellRendererComponent(
                  JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                super.configureListCellRendererComponent(
    ...
    Border errorBorder =
        createCompoundBorder(
            ...
            createCompoundBorder(defaultBorder, createEmptyBorder(1, 1, 1, 1)));
    ...
    add(
        newButton(
            "Sequence", ResourceManager.getIcon("action.export"), evt -> createItemSequence()),
  ...
  public void createItemSequence(List<?> objects) {
    List<ListItem> items =
        IntStream.range(0, objects.size())
            .mapToObj(i -> createItem(objects.get(i), i + 1, 1, objects.size(), objects))
            .collect(toList());
  ...
  public void createItemSequence() {
    ...
    List<ListItem> items =
        context.stream()
            .map(it -> createItem(it, it.intValue(), from, to, context))
            .collect(toList());
