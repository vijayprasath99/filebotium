...
class ExtractTool extends Tool<TableModel> {
  ...
  private Action extractAction =
      newAction(
          "Extract All",
          ResourceManager.getIcon("package.extract"),
          evt -> {
            List<File> archives = ((ArchiveEntryModel) table.getModel()).getArchiveList();
            if (archives.isEmpty()) return;

            File selectedFile =
                showOpenDialogSelectFolder(archives.get(0).getParentFile(), "Extract to ...", evt);
            if (selectedFile == null) return;

            ExtractWorker worker =
                new ExtractWorker(archives, selectedFile, null, true, ConflictAction.AUTO);
          ...
          try {
            ...
            for (FileInfo future : filter == null || forceExtractAll ? outputMapping : selection) {
              if (conflictAction == ConflictAction.AUTO) {
                skip &=
                    (future.toFile().exists() && future.getLength() == future.toFile().length());
              } else {
