...
class AttributeTool extends Tool<TableModel> {
  ...
  protected TableModel createModelInBackground(List<File> root) {
    ...
    for (File file : files) {
      ...
      if (metaObject instanceof Episode) {
        ...
        if (seriesInfo != null) {
          model.addRow(
              String.format("%s::%d", seriesInfo.getDatabase(), seriesInfo.getId()),
              metaObject,
              originalName,
              file);
        }
        ...
        if (movie.getTmdbId() > 0) {
          model.addRow(
              String.format("%s::%d", "TheMovieDB", movie.getTmdbId()),
              metaObject,
              originalName,
              file);
        } else if (movie.getImdbId() > 0) {
          model.addRow(
              String.format("%s::%d", "OMDb", movie.getImdbId()), metaObject, originalName, file);
        }
      }
  ...
  private static class FileAttributesTableModel extends AbstractTableModel {
    ...
    public boolean addRow(Object... row) {
      if (row.length != getColumnCount()) return false;
