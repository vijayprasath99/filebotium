  ...
  private class OpenExpandCollapsePopup extends JPopupMenu {
    ...
    public OpenExpandCollapsePopup() {
      ...
      if (selectedFiles.size() > 0) {
        ...
        add(
            new RevealAction(
                "Reveal Folder",
                selectedFiles.stream().map(File::getParentFile).distinct().collect(toList())));
        addSeparator();
      }
      ...
      add(
          newAction(
              "Collapse all", ResourceManager.getIcon("tree.collapse"), evt -> collapseAll()));
    ...
    private class TrashAction extends AbstractAction {
      ...
      public void actionPerformed(ActionEvent event) {
        ...
        FilterPanel parent =
            (FilterPanel) SwingUtilities.getAncestorOfClass(FilterPanel.class, FileTree.this);
        parent.reload();
      }
    }
    ...
    private void maybeShowPopup(MouseEvent e) {
      if (e.isPopupTrigger()) {
        ...
        if (!getSelectionModel().isPathSelected(path)) {
          // if clicked node is not selected, set selection to this node (and deselect all other
