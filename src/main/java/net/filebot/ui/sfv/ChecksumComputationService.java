...
class ChecksumComputationService {
  ...
  /**
   * Get the number of active executors that are associated with this {@link
  ...
  private class ChecksumComputationExecutor extends ThreadPoolExecutor {
    ...
    public ChecksumComputationExecutor() {
      super(
          ...
          new DefaultThreadFactory("ChecksumComputationPool", Thread.MIN_PRIORITY));
    ...
    protected int getPreferredPoolSize() {
      ...
      // e.g 50 files ~ 1 thread, 200 files ~ 2 threads, 1000 files ~ 3 threads, 40000 files ~ 5
      // threads
      return (int)
          Math.max(1, Math.round(Math.sqrt(threadPoolSize) + Math.log10(getQueue().size()) - 1));
    }
    ...
    public void execute(Runnable command) {
      ...
      if (preferredPoolSize > 0
          && preferredPoolSize <= getMaximumPoolSize()
          && getCorePoolSize() < preferredPoolSize) {
        try {
