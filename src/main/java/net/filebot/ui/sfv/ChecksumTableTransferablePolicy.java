...
class ChecksumTableTransferablePolicy extends BackgroundFileTransferablePolicy<ChecksumCell> {
  ...
  public ChecksumTableTransferablePolicy(
      ChecksumTable table, ChecksumComputationService checksumComputationService) {
    this.table = table;
  ...
  private final ThreadLocal<VerificationTracker> verificationTracker =
      new ThreadLocal<VerificationTracker>();
  ...
  protected void loadVerificationFile(File file, HashType type)
      throws IOException, InterruptedException {
    VerificationFileReader parser =
        new VerificationFileReader(createTextReader(file), type.getFormat());
  ...
  protected void load(File absoluteFile, File relativeFile, File root)
      throws IOException, InterruptedException {
    if (Thread.interrupted()) {
      throw new InterruptedException();
    }
    ...
    if (absoluteFile.isDirectory()) {
      ...
      Map<File, String> hashByVerificationFile =
          verificationTracker.get().getHashByVerificationFile(absoluteFile);
      ...
      for (Entry<File, String> entry : hashByVerificationFile.entrySet()) {
        ...
        ChecksumCell[] verifyCell = {
          new ChecksumCell(name, entry.getKey(), singletonMap(hashType, entry.getValue()))
        };
        publish(verifyCell);
      }
    }
  ...
  protected ChecksumCell createComputationCell(String name, File root, HashType hash) {
    ChecksumCell cell =
        new ChecksumCell(name, root, new ChecksumComputationTask(new File(root, name), hash));
  ...
  private static class VerificationTracker {
          ...
          for (File verificationFile : getChildren(folder, VERIFICATION_FILES)) {
            ...
            cache.put(
                ...
                importVerificationFile(
                    verificationFile, hashType, verificationFile.getParentFile()));
    ...
    /**
     * Completely read a verification file and resolve all relative file paths against a given base
     * folder
     */
    private Map<File, String> importVerificationFile(
        File verificationFile, HashType hashType, File baseFolder) throws IOException {
      VerificationFileReader parser =
          new VerificationFileReader(createTextReader(verificationFile), hashType.getFormat());
      ...
      try {
        while (parser.hasNext()) {
          ...
          // resolve relative path, the hash is probably a substring, so we compact it, for memory
          // reasons
