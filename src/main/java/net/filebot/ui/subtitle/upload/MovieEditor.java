...
class MovieEditor implements TableCellEditor {
  ...
  private String guessQuery(SubtitleMapping mapping) {
    String fn =
        FileUtilities.getName(
            mapping.getVideo() != null ? mapping.getVideo() : mapping.getSubtitle());
  ...
  private List<SubtitleSearchResult> runSearch(SubtitleMapping mapping, JTable table)
      throws Exception {
    String input =
        showInputDialog(
            "Enter movie / series name:", guessQuery(mapping), getFileHint(mapping), table);
  ...
  private void runSelect(
      List<SubtitleSearchResult> options, SubtitleMapping mapping, JTable table) {
    if (options == null) {
      return;
    }
    if (options.isEmpty()) {
    ...
    if (selectedValue != null) {
      ...
      if (mapping.getIdentity() != null
          && mapping.getLanguage() != null
          && mapping.getVideo() != null) {
