...
class SubtitleDownloadComponent extends JComponent {
  ...
  public SubtitleDownloadComponent() {
    ...
    DefaultEventSelectionModel<SubtitlePackage> packageSelection =
        new DefaultEventSelectionModel<SubtitlePackage>(packages);
    ...
    final JList fileList =
        new ListView(createFileListModel()) {
    ...
    // better selection behaviour
    DefaultEventSelectionModel<MemoryFile> fileSelection =
        new DefaultEventSelectionModel<MemoryFile>(files);
    ...
    fileList.setTransferHandler(
        new DefaultTransferHandler(null, memoryFileExportHandler, memoryFileExportHandler));
    ...
    SwingUI.installAction(
        packageList,
        KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0),
        new AbstractAction("Fetch") {
  ...
  protected ListModel createPackageListModel() {
    ...
    MatcherEditor<SubtitlePackage> matcherEditor =
        new TextComponentMatcherEditor<SubtitlePackage>(
            filterEditor,
            new TextFilterator<SubtitlePackage>() {
    ...
    source =
        new ObservableElementList<SubtitlePackage>(
            source, GlazedLists.beanConnector(SubtitlePackage.class));
  ...
  protected ListModel createFileListModel() {
    ...
    // sort by name
    source =
        new SortedList<MemoryFile>(
            source,
            new Comparator<MemoryFile>() {
  ...
  private void fetch(final SubtitlePackage subtitle) {
    ...
    // listen to download
    subtitle.addPropertyChangeListener(
      ...
      for (Object object : selection) {
        ...
        File destination =
            showSaveDialogSelectFile(
                false,
                new File(validateFileName(data.getName())),
                "Save Subtitles as ...",
  ...
  private void export(Object[] selection) {
    try {
      ...
      // just use default values when we can't use a JFC with accessory component (also Swing OSX
      // LaF doesn't seem to support JFileChooser::setAccessory)
      if (Settings.isMacApp()) {
        // COCOA || AWT
        selectedOutputFolder =
            showOpenDialogSelectFolder(
                null,
                "Export Subtitles to Folder (SubRip / UTF-8)",
                new ActionEvent(this, ActionEvent.ACTION_PERFORMED, "Export"));
      } else {
        // Swing
      ...
      if (selectedOutputFolder != null) {
        ...
        for (Object object : selection) {
          ...
          File destination =
              new File(selectedOutputFolder, name + "." + selectedFormat.getFilter().extension());

          SubtitleFormat targetFormat =
              selectedFormat.getFilter().accept(file.getName())
                  ? null
                  : selectedFormat; // check if format conversion is necessary
          writeFile(
              exportSubtitles(file, targetFormat, selectedTimingOffset, selectedEncoding),
              destination);
  ...
  private final Action clearFilterAction =
      new AbstractAction("Clear Filter", ResourceManager.getIcon("edit.clear")) {
  ...
  private final MouseListener packageListMouseHandler =
      new MouseAdapter() {
        ...
        private void maybeShowPopup(MouseEvent e) {
          if (e.isPopupTrigger()) {
            ...
            if (selection.length > 0) {
              ...
              JMenuItem item =
                  contextMenu.add(
                      new AbstractAction("Download", ResourceManager.getIcon("package.fetch")) {
  ...
  private final MouseListener fileListMouseHandler =
      new MouseAdapter() {
          ...
          if (e.isPopupTrigger()) {
            ...
            if (selection.length > 0) {
              ...
              contextMenu.add(
                  newAction(
                      "Preview",
                      ResourceManager.getIcon("action.find"),
                      evt -> open(selection))); // Open
              contextMenu.add(
                  newAction(
                      "Save As...",
                      ResourceManager.getIcon("action.save"),
                      evt -> save(selection))); // Save As...
              contextMenu.add(
                  newAction(
                      "Export...",
                      ResourceManager.getIcon("action.export"),
                      evt -> export(selection))); // Export...
              contextMenu.show(e.getComponent(), e.getX(), e.getY());
            }
          }
        }
