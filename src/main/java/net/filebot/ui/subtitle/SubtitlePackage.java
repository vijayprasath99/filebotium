...
public class SubtitlePackage {
  ...
  public SubtitlePackage(SubtitleProvider provider, SubtitleDescriptor subtitle) {
    ...
    download.addPropertyChangeListener(
  ...
  public void reset() {
    ...
    for (PropertyChangeListener listener :
        old.getPropertyChangeSupport().getPropertyChangeListeners()) {
  ...
  public static class Download extends SwingWorker<List<MemoryFile>, Void> {
    ...
    protected List<MemoryFile> doInBackground() throws Exception {
      ...
      // if we can't extract files from a rar archive, it might actually be a zip file with the
      // wrong extension
