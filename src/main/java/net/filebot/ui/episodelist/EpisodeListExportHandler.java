...
class EpisodeListExportHandler extends FileBotListExportHandler<Episode>
  ...
  public void exportToClipboard(JComponent c, Clipboard clipboard, int action)
    ...
    StringSelection stringSelection =
        new StringSelection(StringUtilities.join(episodeData.getArray(), System.lineSeparator()));
  ...
  public ArrayTransferable<Episode> export(FileBotList<?> list, boolean forceAll) {
    Episode[] selection =
        ((List<?>) list.getListComponent().getSelectedValuesList())
            .stream().map(Episode.class::cast).toArray(Episode[]::new);
