...
public class EpisodeListPanel extends AbstractSearchPanel<EpisodeListProvider, Episode> {
  ...
  public EpisodeListPanel() {
    ...
    searchTextField
        .getSelectButton()
        .addPropertyChangeListener(SelectButton.SELECTED_VALUE, selectButtonListener);

    installAction(
        this,
        KeyStroke.getKeyStroke(KeyEvent.VK_UP, KeyEvent.SHIFT_DOWN_MASK),
        new SpinSeasonAction(1));
    installAction(
        this,
  ...
  protected Collection<String> getHistory(EpisodeListProvider engine) throws Exception {
    ...
    final SearchResult[] index =
        (engine == WebServices.AniDB)
            ? MediaDetection.releaseInfo.getAnidbIndex()
            : MediaDetection.releaseInfo.getTheTVDBIndex();
  ...
  protected EpisodeListRequestProcessor createRequestProcessor() {
    ...
    return new EpisodeListRequestProcessor(
        new EpisodeListRequest(provider, text, season, order, language));
  }
  ;

  private final PropertyChangeListener selectButtonListener =
      new PropertyChangeListener() {
        ...
        public void propertyChange(PropertyChangeEvent evt) {
          ...
          // lock season spinner on "All Seasons" if provider doesn't support fetching of single
          // seasons
  ...
  protected static class EpisodeListRequestProcessor
      extends RequestProcessor<EpisodeListRequest, Episode> {
    ...
    public Collection<Episode> fetch() throws Exception {
      List<Episode> episodes =
          request.provider.getEpisodeList(getSearchResult(), request.order, request.language);
      ...
      if (request.season != ALL_SEASONS) {
        ...
        if (episodeForSeason.isEmpty()) {
          throw new SeasonOutOfBoundsException(
              getSearchResult().getName(), request.season, getLastSeason(episodes));
        }
  ...
  protected static class EpisodeListTab extends FileBotList<Episode> {
    ...
    public EpisodeListTab() {
      ...
      // Guidelines. Specifically: We found that menu items are not visible, except by
      // right-clicking (see screenshot). See the "WYSIWYG (What You See Is What You Get)," "Give
      // Users
          ...
          menu.add(
              newAction(
                  ...
                  evt -> {
                    ...
                    invokeLater(
                        200,
                        () -> SwingEventBus.getInstance().post(exportHandler.export(this, false)));
                  }));
        }
