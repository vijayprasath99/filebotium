...
public class GettingStartedUtil {
    ...
    invokeJavaFX(
        () -> {
          if (webview) {
            ...
            askGettingStartedHelp(); // libjfxwebkit.dylib cannot be deployed on the MAS due to
  ...
  private static void askGettingStartedHelp() {
    ...
    alert.setContentText(
        "If you have never used FileBot before, please have a look at the video tutorials first.");
    ...
    if (isWindowsApp()) {
      ...
      stage
          .getIcons()
          .setAll(
              ResourceManager.getApplicationIconResources()
                  .map(URL::toString)
                  .map(Image::new)
                  .toArray(Image[]::new));
    }
