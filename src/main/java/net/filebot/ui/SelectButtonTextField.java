...
public class SelectButtonTextField<T> extends JComponent {
  ...
  public SelectButtonTextField() {
    ...
    editor.setBorder(
        createMatteBorder(1, 0, 1, 1, ((LineBorder) selectButton.getBorder()).getLineColor()));
  ...
  private final ActionListener textFieldFocusOnClick =
      new ActionListener() {
  ...
  private class CompletionCellRenderer extends DefaultListCellRenderer {
    ...
    public Component getListCellRendererComponent(
      ...
      String highlightText =
          SelectButtonTextField.this
              .getText()
              .substring(0, ((TextFieldComboBoxUI) editor.getUI()).getEditor().getSelectionStart());

      // highlight the matching sequence
      Matcher matcher =
          Pattern.compile(highlightText, Pattern.LITERAL | Pattern.CASE_INSENSITIVE)
      ...
      if (matcher.find()) {
        if (isSelected) {
          ...
          matcher.appendReplacement(
              htmlText,
              "<span style='color: "
                  + SwingUI.toHex(list.getSelectionBackground())
