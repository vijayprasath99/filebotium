...
public abstract class AbstractSearchPanel<S, E> extends JComponent {
  ...
  protected final JPanel tabbedPaneGroup =
      new JPanel(new MigLayout("nogrid, fill, insets 0", "align center", "[fill]8px[pref!]4px"));
  ...
  public AbstractSearchPanel() {
    ...
    JScrollPane historyScrollPane =
        new JScrollPane(historyPanel, VERTICAL_SCROLLBAR_AS_NEEDED, HORIZONTAL_SCROLLBAR_NEVER);
    ...
    setLayout(
        new MigLayout(
            "nogrid, novisualpadding, fill, insets 10px 10px 15px 10px",
            "align 45%",
            "[pref!]10px[fill]"));

    add(searchTextField, "gap 0px:push");
    add(
        new JButton(searchAction),
        "gap 16px, gap after 0px:push, h 2+pref!, id search, sgy button");
    ...
    try {
      ...
      searchTextField
          .getSelectButton()
          .setSelectedIndex(Integer.parseInt(getSettings().get("engine.selected", "0")));
    } catch (Exception e) {
      // log and ignore
      debug.log(Level.WARNING, e.getMessage(), e);
    }
            ...
            new ChangeListener() {
              ...
              public void stateChanged(ChangeEvent e) {
                getSettings()
                    .put(
                        "engine.selected",
                        Integer.toString(searchTextField.getSelectButton().getSelectedIndex()));
              }
            });

    // high-performance auto-completion
    AutoCompleteSupport<String> acs =
  ...
  private final AbstractAction searchAction =
      new AbstractAction("Find", ResourceManager.getIcon("action.find")) {
        ...
        switch (results.size()) {
          case 0:
            log.log(
                Level.WARNING,
                String.format(
                    "'%s' has not been found.", requestProcessor.request.getSearchText()));
            break;
          case 1:
          ...
          default:
            selectedSearchResult =
                requestProcessor.selectSearchResult(
                    results, SwingUtilities.getWindowAncestor(AbstractSearchPanel.this));
            break;
        }
  ...
  private class FetchTask extends SwingWorker<Collection<E>, Void> {
    ...
    public void done() {
      ...
      try {
        ...
        historyPanel.add(
            title,
            requestProcessor.getLink(),
            icon,
            statusMessage,
            String.format("%,d ms", requestProcessor.getDuration()));
  ...
  protected abstract static class RequestProcessor<R extends Request, E> {
    ...
    public String getTitle() {
      if (searchResult != null) return searchResult.getName();
    ...
    protected SearchResult selectSearchResult(
        Collection<? extends SearchResult> searchResults, Window window) throws Exception {
      // multiple results have been found, user must select one
      SelectDialog<SearchResult> selectDialog =
          new SelectDialog<SearchResult>(window, searchResults);
