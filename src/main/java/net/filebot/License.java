...
public class License {
  ...
  public static License parseLicenseText(String psm, boolean strict) {
    try {
      ...
      Instant expires =
          Optional.ofNullable(properties.get("Valid-Until"))
              .map(License::parseExpirationDate)
              .orElse(null);
  ...
  private static void verifyLicense(String id, String psm, boolean strict) throws Exception {
    ...
    Object json =
        cache
            .json(id, i -> new URL("https://license.filebot.net/verify/" + i))
            .fetch(
                (url, modified) -> {
                  ...
                  return WebRequest.post(
                      url, psm.getBytes(UTF_8), "application/octet-stream", getRequestParameters());
                })
            .expire(Cache.ONE_MONTH)
            .get();
  ...
  public static final SystemProperty<File> FILE =
      SystemProperty.of(
          "net.filebot.license", File::new, ApplicationFolder.AppData.resolve("license.txt"));
  public static final MemoizedResource<License> INSTANCE =
      Resource.lazy(() -> parseLicenseFile(FILE.get()));
