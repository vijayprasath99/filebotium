...
public final class StringUtilities {
  ...
  public static <T> Stream<T> streamMatches(
      CharSequence s, Pattern pattern, Function<MatchResult, T> mapper) {
  ...
  public static Stream<String> streamCapturingGroups(MatchResult match) {
    ...
    return IntStream.rangeClosed(1, match.groupCount())
  ...
  public static String join(
      Stream<?> values, CharSequence delimiter, CharSequence prefix, CharSequence suffix) {
    return values
        .map(StringUtilities::asNonEmptyString)
        .filter(Objects::nonNull)
