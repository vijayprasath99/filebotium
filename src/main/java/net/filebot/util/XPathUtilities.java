...
public final class XPathUtilities {
  ...
  public static Node getChild(String nodeName, Node parentNode) {
    if (parentNode == null) {
      ...
      return stream(parentNode.getChildNodes())
          .filter(n -> nodeName.equals(n.getNodeName()))
          .findFirst()
          .orElse(null);
    }
  }
  ...
  /**
   * Get text content of the first child node matching the given node name. Use this method instead
   * of {@link #selectString(String, Object)} whenever xpath support is not required, because it is
   * much faster, especially for large documents.
   *
