...
public class SeparatorBorder extends AbstractBorder {
      ...
      switch (this) {
        ...
        case BOTTOM:
          return new Rectangle2D.Double(
              shape.getX(), shape.getMaxY() - borderWidth, shape.getWidth(), borderWidth);
        case LEFT:
          return new Rectangle2D.Double(shape.getX(), shape.getY(), borderWidth, shape.getHeight());
        case RIGHT:
          return new Rectangle2D.Double(
              shape.getMaxX() - borderWidth, shape.getY(), borderWidth, shape.getHeight());
        default:
          return null;
      }
    }
  ...
  public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
    ...
    Rectangle2D shape =
        position.getRectangle(new Rectangle2D.Double(x, y, width, height), this.borderWidth);

    if (gradientStyle != null && endColor != null)
      g2d.setPaint(gradientStyle.getGradientPaint(shape, beginColor, endColor));
    else g2d.setPaint(beginColor);
