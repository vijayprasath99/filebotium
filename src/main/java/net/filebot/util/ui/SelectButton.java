...
public class SelectButton<T> extends JButton {
  ...
  public T getSelectedValue() {
    if (!selectionModel.isSelected()) return null;
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (hover)
      g2d.setPaint(
          GradientStyle.TOP_TO_BOTTOM.getGradientPaint(bounds, beginColorHover, endColorHover));
    else g2d.setPaint(GradientStyle.TOP_TO_BOTTOM.getGradientPaint(bounds, beginColor, endColor));
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      for (T value : model) {
        SelectPopupMenuItem item =
            new SelectPopupMenuItem(
                labelProvider.getText(value), labelProvider.getIcon(value), value);

        if (value == getSelectedValue()) item.setSelected(true);
