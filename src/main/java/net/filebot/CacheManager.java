...
public class CacheManager {
    ...
    try {
      ...
      this.manager =
          net.sf.ehcache.CacheManager.create(
              new Configuration()
                  .diskStore(new DiskStoreConfiguration().path(diskStore.getPath())));
    } catch (IOException e) {
  ...
  private void clearDiskStore(File cache) {
    getChildren(cache, FILES).stream()
  ...
  private File acquireDiskStore() throws IOException {
    for (int i = 0; i < 10; i++) {
      ...
      if (lock != null) {
        ...
        if (cacheRevision != applicationRevision && applicationRevision > 0 && !isNewCache) {
          debug.warning(
              format(
                  "Current application revision (r%d) does not match cache revision (r%d): reset cache",
                  applicationRevision, cacheRevision));
    ...
    throw new IOException(
        "Unable to acquire cache lock: " + ApplicationFolder.Cache.get().getAbsolutePath());
  }
