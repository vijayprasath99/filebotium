...
public class TheTVDBClientV1 extends AbstractEpisodeListProvider implements ArtworkProvider {
  ...
  protected SeriesData fetchSeriesData(SearchResult series, SortOrder sortOrder, Locale locale)
    ...
    Document dom =
        getXmlResource(
            MirrorType.XML,
            "series/" + series.getId() + "/all/" + getLanguageCode(locale) + ".xml");
    ...
    for (Node node : selectNodes("Data/Episode", dom)) {
      ...
      if (seasonNumber == null || seasonNumber == 0) {
        ...
        Integer specialNumber =
            (episodeNumber != null)
                ? episodeNumber
                : filterBySeason(specials, seasonNumber).size() + 1;
        specials.add(
            new Episode(
                seriesInfo.getName(),
                seasonNumber,
  ...
  public SearchResult lookupByID(int id, Locale locale) throws Exception {
    ...
    return getLookupCache("id", locale)
        .computeIfAbsent(
            id,
            it -> {
              Document dom =
                  getXmlResource(
                      MirrorType.XML, "series/" + id + "/all/" + getLanguageCode(locale) + ".xml");
  ...
  public SearchResult lookupByIMDbID(int imdbid, Locale locale) throws Exception {
    ...
    return getLookupCache("imdbid", locale)
        .computeIfAbsent(
            imdbid,
            it -> {
              Document dom =
                  getXmlResource(
                      MirrorType.SEARCH,
                      "GetSeriesByRemoteID.php?imdbid="
                          + imdbid
                          + "&language="
                          + getLanguageCode(locale));
  ...
  protected String getMirror(MirrorType mirrorType) throws Exception {
    ...
    synchronized (mirrors) {
      ...
      if (mirrors.isEmpty()) {
        ...
        Map<MirrorType, List<String>> mirrorLists =
            streamNodes("Mirrors/Mirror", dom)
                .flatMap(
                    node -> {
                      String mirror = getTextContent("mirrorpath", node);
                      int typeMask = Integer.parseInt(getTextContent("typemask", node));

                      return MirrorType.fromTypeMask(typeMask).stream()
                          .collect(toMap(m -> m, m -> mirror))
                          .entrySet()
                          .stream();
                    })
                .collect(
                    groupingBy(
                        Entry::getKey, MirrorType::newMap, mapping(Entry::getValue, toList())));
  ...
  protected URL getResource(MirrorType mirror, String path) throws Exception {
    StringBuilder url =
        new StringBuilder(getMirror(mirror)).append('/').append(mirror.prefix()).append('/');
  ...
  protected static enum MirrorType {
    ...
    public static EnumSet<MirrorType> fromTypeMask(int mask) {
      ...
      return EnumSet.of(SEARCH, XML, BANNER).stream()
          .filter(
              m -> {
  ...
  @Override
  public TheTVDBSeriesInfo getSeriesInfo(SearchResult searchResult, Locale language)
      throws Exception {
  ...
  public List<Artwork> getArtwork(int id, String category, Locale locale) throws Exception {
    ...
    return streamNodes("//Banner", dom)
        .map(
            node -> {
              try {
                ...
                return new Artwork(
                    Stream.of(type, subKey, season),
                    new URL(mirror, fileName),
        ...
        .filter(it -> it.getTags().contains(category))
        .collect(toList());
  }
  ...
  protected TypedCache<SearchResult> getLookupCache(String type, Locale language) {
    ...
    return Cache.getCache(
            getName() + "_" + "lookup" + "_" + type + "_" + language, CacheType.Monthly)
