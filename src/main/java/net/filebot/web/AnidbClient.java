...
public class AnidbClient extends AbstractEpisodeListProvider {
  ...
  private static final FloodLimit REQUEST_LIMIT =
      new FloodLimit(2, 5, TimeUnit.SECONDS); // no more than 2 requests within a 5 second window
  ...
  private final Resource<LocalSearch<SearchResult>> localIndex =
      Resource.lazy(
          () -> new LocalSearch<SearchResult>(getAnimeTitles(), SearchResult::getEffectiveNames));
  ...
  @Override
  protected SeriesData fetchSeriesData(SearchResult anime, SortOrder sortOrder, Locale locale)
      throws Exception {
    ...
    // * limit to 5 genres
    seriesInfo.setGenres(
        streamNodes("anime/categories/category", dom)
            .map(
                categoryNode -> {
    ...
    String animeTitle =
        selectString(
            "anime/titles/title[@type='official' and @lang='" + getLanguageCode(locale) + "']",
            dom);
        ...
        if (type == 1) {
          ...
          episodes.add(
              new Episode(
                  ...
                  airdate,
                  id,
                  new SeriesInfo(seriesInfo))); // normal episode, no seasons for anime
        } else {
          episodes.add(
              new Episode(
                  animeTitle,
                  ...
                  airdate,
                  id,
                  new SeriesInfo(seriesInfo))); // special episode
        }
      }
    }
    ...
    if (episodes.isEmpty()) {
      debug.fine(
          format(
              "No episode data: %s (%d) => %s", anime, anime.getId(), getResource(anime.getId())));
    }
  ...
  private Document getXmlResource(int aid) throws Exception {
    ...
    return cache
        .xml(aid, this::getResource)
        .fetch(withPermit(fetchIfModified(), r -> REQUEST_LIMIT.acquirePermit()))
  ...
  private URL getResource(int aid) throws Exception {
    // e.g.
    // http://api.anidb.net:9001/httpapi?request=anime&client=filebot&clientver=1&protover=1&aid=4521
    return new URL(
  ...
  /**
   * This method is overridden in {@link net.filebot.WebServices.AnidbClientWithLocalSearch} to
   * fetch the Anime Index from our own host and not anidb.net
   */
  public SearchResult[] getAnimeTitles() throws Exception {
    // get data file (unzip and cache)
    byte[] bytes =
        getCache("root")
            .bytes("anime-titles.dat.gz", n -> new URL("http://anidb.net/api/" + n))
            .get();

    // <aid>|<type>|<language>|<title>
    // type: 1=primary title (one per anime), 2=synonyms (multiple per anime), 3=shorttitles
    // (multiple per anime), 4=official title (one per language)
          ...
          .forEach(
              line -> {
                ...
                if (matcher.matches()) {
                  ...
                  if (aid > 0
                      && title.length() > 0
                      && typeOrder.contains(type)
                      && languageOrder.contains(language)) {
                    ...
                    if (type.equals("3")
                        && (title.length() < 5
                            ...
                            || Character.isUpperCase(title.charAt(title.length() - 1)))) {
                      return;
                    }

                    entriesByAnime
                        .computeIfAbsent(aid, k -> new ArrayList<Object[]>())
                        .add(
                            new Object[] {
                              typeOrder.indexOf(type), languageOrder.indexOf(language), title
                            });
                  }
                }
              });
    ...
    return entriesByAnime.entrySet().stream()
        .map(
            it -> {
              List<String> names =
                  it.getValue().stream()
