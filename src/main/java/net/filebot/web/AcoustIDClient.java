...
public class AcoustIDClient implements MusicIdentificationService {
  ...
  public Map<File, AudioTrack> lookup(Collection<File> files) throws Exception {
    ...
    for (File file : files) {
      ...
      // sanity check
      if (!fp.containsKey(ChromaprintField.DURATION)
          || !fp.containsKey(ChromaprintField.FINGERPRINT)) continue;
        ...
        getCache()
            .computeIfAbsent(
                ...
                it -> {
                  ...
                  URL url =
                      new URL(
                          "http://api.acoustid.org/v2/lookup?client="
                              + apikey
  ...
  public AudioTrack parseResult(String json, final int targetDuration) throws IOException {
    ...
    for (Object result : getArray(data, "results")) {
      ...
      return streamJsonObjects(result, "recordings")
          .sorted(
              (r1, r2) -> {
                Integer i1 = getInteger(r1, "duration");
                Integer i2 = getInteger(r2, "duration");
                return Double.compare(
                    i1 == null ? Double.NaN : Math.abs(i1 - targetDuration),
                    i2 == null ? Double.NaN : Math.abs(i2 - targetDuration));
              })
          .map(
              (Map<?, ?> recording) -> {
                try {
                  ...
                  if (artist == null || title == null || releaseGroup.isEmpty()) return null;
                  ...
                  return streamJsonObjects(releases)
                      .map(
                          release -> {
                            ...
                            try {
                              ...
                              thisRelease.albumReleaseDate =
                                  new SimpleDate(
                                      getInteger(date, "year"),
                                      getInteger(date, "month"),
                                      getInteger(date, "day"));
                            } catch (Exception e) {
                              thisRelease.albumReleaseDate = null;
                            }

                            if (thisRelease.albumReleaseDate == null
                                || thisRelease.albumReleaseDate.getTimeStamp()
                                    >= (audioTrack.albumReleaseDate == null
                                        ? Long.MAX_VALUE
                                        : audioTrack.albumReleaseDate.getTimeStamp())) {
                              return null;
                            }
                            ...
                            try {
                              thisRelease.albumArtist =
                                  getString(getFirstMap(releaseGroup, "artists"), "name");
                            } catch (Exception e) {
                              thisRelease.albumArtist = null;
                            }
                            thisRelease.trackTitle = getString(track, "title");

                            if (!"Various Artists".equalsIgnoreCase(thisRelease.albumArtist)
                                && (thisRelease.album == null
                                    || !thisRelease.album.contains("Greatest Hits"))) {
                      ...
                      .findFirst()
                      .orElse(audioTrack); // default to simple music info if extended info is not
                  // available
                } catch (Exception e) {
  ...
  public String getChromaprintCommand() {
    // use fpcalc executable path as specified by the cmdline or default to "fpcalc" and let the
    // shell figure it out
  ...
  public Map<ChromaprintField, String> fpcalc(File file) throws IOException, InterruptedException {
    Map<ChromaprintField, String> output =
        new EnumMap<ChromaprintField, String>(ChromaprintField.class);
