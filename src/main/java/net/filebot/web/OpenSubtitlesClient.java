...
public class OpenSubtitlesClient
    implements SubtitleProvider, VideoHashSubtitleService, MovieIdentificationService {
  ...
  public OpenSubtitlesClient(String name, String version) {
    this.xmlrpc =
        new OpenSubtitlesXmlRpcWithRetryAndFloodLimit(
            String.format("%s v%s", name, version), 2, 3000);
  }
  ...
  public List<SubtitleSearchResult> search(String query) throws Exception {
    throw new UnsupportedOperationException(
        "XMLRPC::SearchMoviesOnIMDB has been banned due to abuse");
  }
  ...
  public List<SubtitleDescriptor> getSubtitleList(SubtitleSearchResult searchResult, Locale locale)
      throws Exception {
    return getSubtitleList(searchResult, -1, -1, locale);
  }

  @Override
  public List<SubtitleDescriptor> getSubtitleList(
      SubtitleSearchResult searchResult, int[][] episodeFilter, Locale locale) throws Exception {
    // no filter
              ...
              s -> {
                try {
                  ...
                  throw new RuntimeException(
                      String.format(
                          "Failed to retrieve subtitle list for season: %s S%02d [%s]",
                          searchResult, s, locale),
                      e);
                }
              })
          .distinct()
        ...
        .flatMap(
            ii -> {
              try {
                ...
                throw new RuntimeException(
                    String.format(
                        "Failed to retrieve subtitle list for episode: %s %s [%s]",
                        searchResult, asList(ii), locale),
                    e);
              }
            })
        .distinct()
  ...
  public synchronized List<SubtitleDescriptor> getSubtitleList(
      SubtitleSearchResult searchResult, int season, int episode, Locale locale) throws Exception {
    Query query =
        Query.forImdbId(searchResult.getImdbId(), season, episode, getLanguageFilter(locale));
  ...
  @Override
  public Map<File, List<SubtitleDescriptor>> getSubtitleList(File[] files, Locale locale)
      throws Exception {
    Map<File, List<SubtitleDescriptor>> results =
        new HashMap<File, List<SubtitleDescriptor>>(files.length);
  ...
  protected Map<File, List<SubtitleDescriptor>> getSubtitleList(
      File[] files, Function<File, Query> queryMapper) throws Exception {
    Map<File, List<SubtitleDescriptor>> results =
        new HashMap<File, List<SubtitleDescriptor>>(files.length);
  ...
  public Map<File, List<SubtitleDescriptor>> getSubtitleListByHash(File[] files, Locale locale)
      throws Exception {
  ...
  public Map<File, List<SubtitleDescriptor>> getSubtitleListByTag(File[] files, Locale locale)
      throws Exception {
    return getSubtitleList(
        files,
        f -> {
  ...
  @Override
  public synchronized CheckResult checkSubtitle(File videoFile, File subtitleFile)
      throws Exception {
    // require login
    login();
    ...
    // TryUploadResponse: false => [{HashWasAlreadyInDb=1, MovieKind=movie, IDSubtitle=3167446,
    // MoviefilenameWasAlreadyInDb=1, ISO639=en, MovieYear=2007, SubLanguageID=eng, MovieName=Blades
    // of Glory, MovieNameEng=, IDMovieImdb=445934}]
  ...
  @Override
  public synchronized void uploadSubtitle(
      Object identity, Locale locale, File[] videoFile, File[] subtitleFile) throws Exception {
    int imdbid = -1;
    try {
  ...
  protected SubFile getSubFile(File videoFile, File subtitleFile, boolean content)
      throws IOException {
  ...
  public synchronized Map<File, Movie> getMovieDescriptors(
      Collection<File> movieFiles, Locale locale) throws Exception {
    // create result array
    Map<File, Movie> results = new HashMap<File, Movie>();

    // make sure we don't get mismatches by making sure the hash has not been confirmed numerous
    // times
    int minSeenCount = 20;
  ...
  public URI getSubtitleListLink(SubtitleSearchResult searchResult, Locale locale) {
    return URI.create(
        String.format(
            "http://www.opensubtitles.org/en/search/imdbid-%d/sublanguageid-%s",
            searchResult.getImdbId(), getSubLanguageID(locale)));
  }
  ...
  protected final Timer logoutTimer =
      new Timer() {
  ...
  protected synchronized Map<String, String> getSubLanguageMap() throws Exception {
    ...
    Map<?, ?> m =
        (Map<?, ?>) cache.computeIfAbsent("subLanguageMap", k -> xmlrpc.getSubLanguages());

    // add additional language aliases for improved compatibility
    Map<String, Locale> additionalLanguageMappings =
        MediaDetection.releaseInfo.getLanguageMap(Locale.ENGLISH);
    ...
    m.forEach(
        (k, v) -> {
          ...
          subLanguageMap.put(
              subLanguageID,
              subLanguageID); // add reverse mapping as well for improved compatibility
          ...
          for (String key : new String[] {subLanguageID, languageCode}) {
            ...
            if (locale != null) {
              for (String identifier :
                ...
                if (identifier != null
                    && identifier.length() > 0
                    && !subLanguageMap.containsKey(identifier.toLowerCase())) {
  ...
  protected String[] getLanguageFilter(Locale locale) {
    return locale == null || locale.getLanguage().isEmpty()
        ? new String[0]
        : new String[] {getSubLanguageID(locale)};
  }
  ...
  protected static class OpenSubtitlesXmlRpcWithRetryAndFloodLimit extends OpenSubtitlesXmlRpc {
    ...
    public OpenSubtitlesXmlRpcWithRetryAndFloodLimit(
        String useragent, int retryCountLimit, long retryWaitTime) {
      super(useragent);
