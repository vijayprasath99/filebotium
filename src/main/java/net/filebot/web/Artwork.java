...
public class Artwork implements Serializable {
  ...
  public Artwork(Stream<?> tags, URL url, Locale language, Double rating) {
    ...
    this.url = url;
    this.language =
        language == null || language.getLanguage().isEmpty() ? null : language.getLanguage();
    this.rating = rating;
  }
  ...
  public boolean matches(Object... tags) {
    ...
    return stream(tags)
        ...
        .allMatch(
            tag -> {
              return stream(this.tags).anyMatch(tag::equalsIgnoreCase)
                  || tag.equalsIgnoreCase(language);
            });
  }
  ...
  public String toString() {
    return Stream.of(String.join("/", tags), language, rating, url)
        .filter(Objects::nonNull)
        .map(Object::toString)
        .collect(joining(", ", "[", "]"));
  }

  public static final Comparator<Artwork> RATING_ORDER =
      Comparator.comparing(Artwork::getRating, reverseOrder());
}
