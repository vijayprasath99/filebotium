...
public class LocalSearch<T> {
  ...
  private final StringMetric metric =
      with(new BlockDistance<String>()).tokenize(qGramWithPadding(3)).build();
    ...
    return IntStream.range(0, objects.length)
        .mapToObj(
            i -> {
              ...
              double similarity =
                  field.stream().mapToDouble(it -> metric.compare(query, it)).max().orElse(0);

              return match || similarity > resultMinimumSimilarity
                  ? new SimpleImmutableEntry<T, Double>(object, similarity)
                  : null;
            })
