...
public abstract class AbstractEpisodeListProvider implements EpisodeListProvider {
  ...
  protected abstract List<SearchResult> fetchSearchResult(String query, Locale locale)
      throws Exception;

  protected abstract SeriesData fetchSeriesData(
      SearchResult searchResult, SortOrder sortOrder, Locale locale) throws Exception;

  @Override
  public List<SearchResult> search(String query, Locale language) throws Exception {
    return getSearchCache(language)
        .computeIfAbsent(
            query,
            it -> {
  ...
  public List<Episode> getEpisodeList(
      SearchResult searchResult, SortOrder sortOrder, Locale language) throws Exception {
  ...
  protected SeriesData getSeriesData(SearchResult searchResult, SortOrder order, Locale language)
      throws Exception {
    ...
    return getDataCache(requestOrder, requestLanguage)
        .computeIfAbsent(
            searchResult.getId(),
            it -> {
