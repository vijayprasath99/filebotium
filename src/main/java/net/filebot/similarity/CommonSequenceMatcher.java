...
public class CommonSequenceMatcher {
  ...
  public CommonSequenceMatcher(
      Collator collator, int commonSequenceMaxStartIndex, boolean returnFirstMatch) {
  ...
  private final Map<String, CollationKey> collationKeyDictionary =
      synchronizedMap(new HashMap<String, CollationKey>(64, 4));
  ...
  protected <E extends Comparable<E>> E[] firstCommonSequence(
    ...
    for (int i = 0; i < seq1.length && i <= maxStartIndex; i++) {
      for (int j = 0; j < seq2.length && j <= maxStartIndex; j++) {
        ...
        while ((i + len < seq1.length)
            && (j + len < seq2.length)
            && (seq1[i + len].compareTo(seq2[j + len]) == 0)) {
          len++;
        }
