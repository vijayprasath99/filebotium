...
public class DateMatcher {
  ...
  protected DatePattern[] compile(String[] pattern, DateFilter sanity, Locale... locale) {
    return StreamEx.of(pattern)
        .flatMap(
            dateFormat -> {
              return StreamEx.of(locale)
                  .distinct(Locale::getLanguage)
                  .map(
                      formatLocale -> {
                        String regex =
                            StreamEx.split(dateFormat, DateFormatPattern.DELIMITER)
                                .map(g -> getPatternGroup(g, formatLocale))
                                .joining("\\D", "(?<!\\p{Alnum})", "(?!\\p{Alnum})");
  ...
  protected String getMonthNamePatternGroup(TextStyle style, Locale locale) {
    return StreamEx.of(Month.values())
        .map(m -> m.getDisplayName(style, locale))
        .map(Pattern::quote)
        .joining("|", "(", ")");
  }
