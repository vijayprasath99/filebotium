...
public class EpisodeMatcher extends Matcher<File, Object> {
  ...
  @Override
  protected void deepMatch(Collection<Match<File, Object>> possibleMatches, int level)
      ...
      if (uniqueFiles.equals(uniqueEpisodes)) {
        ...
        if (episodes.size() > 1) {
          Episode[] episodeSequence =
              episodes.stream().sorted(episodeComparator()).distinct().toArray(Episode[]::new);
  ...
  private final SeasonEpisodeMatcher seasonEpisodeMatcher =
      new SmartSeasonEpisodeMatcher(SeasonEpisodeMatcher.LENIENT_SANITY, false);
  ...
  private boolean isMultiEpisode(Episode[] episodes) {
    ...
    Function<Episode, Integer> number =
        stream(episodes).allMatch(e -> e.getSpecial() == null)
            ? e -> e.getEpisode()
            : e -> e.getSpecial();
    ...
    for (Episode it : episodes) {
      ...
      // non-sequential next episode index breaks the chain (same episode is OK since DVD numbering
      // allows for multiple episodes to share the same SxE numbers)
      if (seqIndex != null) {
