...
public class Matcher<V, C> {
  ...
  public Matcher(
      ...
      Collection<? extends C> candidates,
      boolean strict,
  ...
  public synchronized List<Match<V, C>> match() throws InterruptedException {
    ...
    List<Match<V, C>> possibleMatches =
        new ArrayList<Match<V, C>>(values.size() * candidates.size());
  ...
  protected void deepMatch(Collection<Match<V, C>> possibleMatches, int level)
    ...
    if (level >= metrics.length || possibleMatches.isEmpty()) {
      ...
      if (!strict) {
        // order alphabetically to get more predictable matching (when no matching is possible
        // anymore)
        List<Match<V, C>> rest = new ArrayList<Match<V, C>>(possibleMatches);
        sort(
            rest,
            new Comparator<Match<V, C>>() {
    ...
    for (Set<Match<V, C>> matchesWithEqualSimilarity :
        mapBySimilarity(possibleMatches, metrics[level]).values()) {
  ...
  protected void removeCollected(Collection<Match<V, C>> matches) {
    for (Iterator<Match<V, C>> iterator = matches.iterator(); iterator.hasNext(); ) {
      if (!disjointMatchCollection.disjoint(iterator.next())) iterator.remove();
    }
  }

  protected SortedMap<Float, Set<Match<V, C>>> mapBySimilarity(
      Collection<Match<V, C>> possibleMatches, SimilarityMetric metric)
      throws InterruptedException {
    // map sorted by similarity descending
    SortedMap<Float, Set<Match<V, C>>> similarityMap =
        new TreeMap<Float, Set<Match<V, C>>>(Collections.reverseOrder());

    // use metric on all matches
    for (Match<V, C> possibleMatch : possibleMatches) {
      float similarity =
          metric.getSimilarity(possibleMatch.getValue(), possibleMatch.getCandidate());
  ...
  protected List<Match<V, C>> disjointMatches(Collection<Match<V, C>> collection) {
    ...
    for (List<Match<V, C>> matchListForValue : matchesByValue.values()) {
      ...
      if (matchListForValue.size() == 1
          && matchListForValue.equals(
              matchesByCandidate.get(matchListForValue.get(0).getCandidate()))) {
        // match is disjoint :)
