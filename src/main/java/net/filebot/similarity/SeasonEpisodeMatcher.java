...
public class SeasonEpisodeMatcher {
  ...
  public static final SeasonEpisodeFilter LENIENT_SANITY =
      new SeasonEpisodeFilter(99, 999, 9999, 1970, 2100);
  public static final SeasonEpisodeFilter DEFAULT_SANITY =
      new SeasonEpisodeFilter(50, 50, 1000, 1970, 2100);
  public static final SeasonEpisodeFilter STRICT_SANITY =
      new SeasonEpisodeFilter(10, 30, -1, -1, -1);
  ...
  public SeasonEpisodeMatcher(SeasonEpisodeFilter sanity, boolean strict) {
    // define variables
    SeasonEpisodePattern Season_00_Episode_00,
        S00E00SEQ,
        S00E00,
        SxE1_SxE2,
        SxE,
    ...
    Season_00_Episode_00 =
        new SeasonEpisodePattern(
            null,
            "(?<!\\p{Alnum})(?i:season|series)[^\\p{Alnum}]{0,3}(\\d{1,4})[^\\p{Alnum}]{0,3}(?i:episode)[^\\p{Alnum}]{0,3}((\\d{1,3}(\\D|$))+)[^\\p{Alnum}]{0,3}(?!\\p{Digit})",
            m -> {
    ...
    S00E00SEQ =
        new SeasonEpisodePattern(
            null,
            "(?<!\\p{Alnum}|[-])[Ss](\\d{1,2}|\\d{4})[Ee](\\d{2,3})[-][Ee](\\d{2,3})(?!\\p{Alnum}|[-])",
            m -> {
              return range(m.group(1), m.group(2), m.group(3));
            });

    // match patterns like S01E01, s01e02, ... [s01]_[e02], s01.e02, s01e02a, s2010e01 ...
    // s01e01-02-03-04, [s01]_[e01-02-03-04] ...
    S00E00 =
        new SeasonEpisodePattern(
            null,
            "(?<!\\p{Digit})[Ss](\\d{1,2}|\\d{4})[^\\p{Alnum}]{0,3}(?i:ep|e|p|-)(((?<=[^._ ])[Ee]?[Pp]?\\d{1,3}(\\D|$))+)",
            m -> {
    ...
    SxE1_SxE2 =
        new SeasonEpisodePattern(
            sanity,
            "(?<!\\p{Alnum})(\\d{1,2}x\\d{2}([-._ ]\\d{1,2}x\\d{2})+)(?!\\p{Digit})",
            m -> {
    ...
    EP0 =
        new SeasonEpisodePattern(
            sanity,
            "(?<!\\p{Alnum})(\\d{2}|\\d{4})?[\\P{Alnum}]{0,3}(((?i:e|ep|episode|p|part)[\\P{Alnum}]{0,3}\\d{1,3})+)(?!\\p{Digit})",
            m -> {
              return multi(m.group(1), m.group(2));
            });
    ...
    E1of2 =
        new SeasonEpisodePattern(
            sanity,
            "(?<!\\p{Alnum})(\\d{1,2})[^._ ]?(?i:of)[^._ ]?(\\d{1,2})(?!\\p{Digit})",
            m -> {
    ...
    if (strict) {
      patterns =
          new SeasonEpisodeParser[] {
            Season_00_Episode_00, S00E00SEQ, S00E00, SxE1_SxE2, SxE, Dot101
          };
    } else {
      patterns =
          new SeasonEpisodeParser[] {
            ...
            Dot101,
            E01E02SEQ,
            new SeasonEpisodeUnion(EP0, Num101_TOKEN, E1of2),
            Num101_SUBSTRING
          };
    }
  ...
  protected List<SxE> multi(String season, String... episodes) {
    ...
    return stream(episodes)
        .flatMap(e -> matchIntegers(e).stream())
        .map(e -> new SxE(s, e))
        .collect(toList());
  }
  ...
  protected List<SxE> range(String season, String... episodes) {
    IntSummaryStatistics stats =
        stream(episodes)
            .flatMap(s -> matchIntegers(s).stream())
            .mapToInt(i -> i)
            .summaryStatistics();

    // range patterns without season are more prone to false positives, so we need to do some extra
    // sanity checks (e.g. Episode 01-50 is probably not a multi-episode but some sort of season
    // pack)
  ...
  protected List<SxE> numbers(String head, String... tail) {
    ...
    // interpret match both ways, as SxE match as well as episode number only match if it's not an
    // double episode
    if (tail.length == 1) {
   ...
   * @return the matches returned by the first pattern that returns any matches for this string, or
   *     null if no pattern returned any matches
   */
  ...
  public static class SxE implements Comparable<SxE> {
    ...
    public int compareTo(SxE other) {
      return season < other.season
          ...
          : season != other.season
              ? 1
              : episode < other.episode ? -1 : episode != other.episode ? 1 : 0;
    }
  ...
  public static class SeasonEpisodeFilter {
    ...
    public SeasonEpisodeFilter(
        int seasonLimit,
        int seasonEpisodeLimit,
        int absoluteEpisodeLimit,
        int seasonYearBegin,
        int seasonYearEnd) {
    ...
    boolean filter(SxE sxe) {
      return (sxe.season >= 0
              && (sxe.season < seasonLimit
                  || (sxe.season > seasonYearBegin && sxe.season < seasonYearEnd))
              && sxe.episode < seasonEpisodeLimit)
          || (sxe.season < 0 && sxe.episode < absoluteEpisodeLimit);
    }
    ...
    public boolean filter(SxE value, List<SxE> sequence) {
      return filter(value)
          && sequence.stream()
                  .filter(
                      other -> {
                        return (value.season == SxE.UNDEFINED) == (other.season == SxE.UNDEFINED)
  ...
  public static class SeasonEpisodePattern implements SeasonEpisodeParser {
    ...
    public SeasonEpisodePattern(
        SeasonEpisodeFilter sanity, String pattern, Function<MatchResult, List<SxE>> process) {
