...
public final class Settings {
  ...
  private static boolean isApplicationDeployment(String... ids) {
    ...
    for (String id : ids) {
      if (current != null && current.equals(id)) return true;
    }
    return false;
  }
  ...
  public static LicenseModel getLicenseModel() {
    if (isUWP()) return LicenseModel.MicrosoftStore;
    if (isMacSandbox()) return LicenseModel.MacAppStore;
  ...
  public static String getAppStoreName() {
    if (isMacApp()) return "Mac App Store";
    if (isWindowsApp()) return "Microsoft Store";
    if (isUbuntuApp()) return "Ubuntu Software Center";

    return null;
  }

  public static String getAppStoreLink() {
    if (isMacApp()) return getApplicationProperty("link.mas");
    if (isWindowsApp()) return getApplicationProperty("link.mws");
    if (isUbuntuApp()) return getApplicationProperty("link.usc");
  ...
  public static String getWindowTitle() {
    return isAutoUpdateEnabled()
        ? getApplicationName()
        : String.format("%s %s", getApplicationName(), getApplicationVersion());
  }

  public static String getApplicationIdentifier() {
    return String.format(
        "%s %s (r%d)",
        getApplicationName(), getApplicationVersion(), getApplicationRevisionNumber());
  }

  public static String getJavaRuntimeIdentifier() {
    return String.format(
        "%s %s", System.getProperty("java.runtime.name"), System.getProperty("java.version"));
  }
