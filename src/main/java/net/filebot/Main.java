...
public class Main {
  ...
  private static void startUserInterface(ArgumentBean args) {
    ...
    if (isMacSandbox()) {
      panels =
          stream(panels).filter(p -> !p.getName().equals("Subtitles")).toArray(PanelBuilder[]::new);
    }

    JFrame frame = panels.length > 1 ? new MainFrame(panels) : new SinglePanelFrame(panels[0]);
    try {
      restoreWindowBounds(
          frame, Settings.forPackage(MainFrame.class)); // restore previous size and location
    } catch (Exception e) {
  ...
  private static void checkGettingStarted() throws Exception {
    PreferencesEntry<String> started =
        Settings.forPackage(Main.class).entry("getting.started").defaultValue("0");
    if ("0".equals(started.getValue())) {
      ...
      SwingUtilities.invokeLater(
          () -> openGettingStarted("show".equals(System.getProperty("application.help"))));
    }
  }
  ...
  public static void initializeSystemProperties(ArgumentBean args) {
    System.setProperty(
        "http.agent", String.format("%s/%s", getApplicationName(), getApplicationVersion()));
    ...
    if (args.runCLI()) {
      ...
      try {
        Handler errorLogHandler =
            createSimpleFileHandler(ApplicationFolder.AppData.resolve("error.log"), Level.WARNING);
