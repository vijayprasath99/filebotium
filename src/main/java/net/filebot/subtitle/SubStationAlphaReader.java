...
public class SubStationAlphaReader extends SubtitleReader {
  ...
  private final Pattern drawingTags = Pattern.compile("\\\\[p][0-4]"); // ignore drawing commands
  // (http://docs.aegisub.org/3.2/ASS_Tags/#drawing-commands)
  ...
  private void readFormat() throws Exception {
    // read format line (e.g. Format: Marked, Start, End, Style, Name, MarginL, MarginR, MarginV,
    // Effect, Text)
  ...
  public SubtitleElement readNext() throws Exception {
    ...
    if (event.length < 2 || !event[0].equals("Dialogue")) return null;
    ...
    if (drawingTags.matcher(text).find()) return null;
