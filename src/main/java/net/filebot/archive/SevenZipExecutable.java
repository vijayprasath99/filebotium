...
public class SevenZipExecutable implements ArchiveExtractor {
  ...
  protected String get7zCommand() {
    // use 7z executable path as specified by the cmdline or default to "7z" and let the shell
    // figure it out
      ...
      if (returnCode == 0) {
        ...
        throw new IOException(
            String.format(
                ...
                get7zCommand(), returnCode, SPACE.matcher(output).replaceAll(" ").trim()));
      }
  ...
  public void extract(File outputDir) throws IOException {
    ...
    execute(
        get7zCommand(), "x", "-y", "-aos", archive.getPath(), "-o" + outputDir.getCanonicalPath());
  }
  ...
  public void extract(File outputDir, FileFilter filter) throws IOException {
    ...
    Stream<String> selection =
        listFiles().stream().filter(f -> filter.accept(f.toFile())).map(f -> f.getPath());
