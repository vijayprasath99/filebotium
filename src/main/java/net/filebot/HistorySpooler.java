    ...
    Runtime.getRuntime()
        .addShutdownHook(
            new Thread(
                ...
                "HistorySpoolerShutdownHook")); // commit session history on shutdown
  }
        ...
        History history =
            History.importHistory(
                new CloseShieldInputStream(
                    newInputStream(channel))); // keep JAXB from closing the stream
        ...
        try (FileLock lock = channel.lock()) {
          ...
          History.exportHistory(
              ...
              new CloseShieldOutputStream(
                  newOutputStream(channel))); // keep JAXB from closing the stream
