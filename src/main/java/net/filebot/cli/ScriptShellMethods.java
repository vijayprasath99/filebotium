...
public class ScriptShellMethods {
  ...
  public static File getAt(File self, Range<?> range) {
    return new File(
        DefaultGroovyMethods.getAt(FileUtilities.listPath(self), range).stream()
            .map(File::getName)
  ...
  public static List<File> listFiles(File self, Closure<?> closure) {
    return FileUtilities.getChildren(
        self, (FileFilter) DefaultTypeTransformation.castToType(closure, FileFilter.class), null);
  }
  ...
  public static boolean isClutter(File self) {
    return MediaDetection.getClutterTypeFilter().accept(self)
        || MediaDetection.getClutterFileFilter().accept(self);
  }
    ...
    Files.walkFileTree(
        ...
        new SimpleFileVisitor<Path>() {
          ...
          public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs)
            ...
            if (FileUtilities.getChildren(folder, VIDEO_FILES).size() > 0
                || MediaDetection.isDiskFolder(folder)) {
              folders.add(folder);
              return FileVisitResult.SKIP_SUBTREE;
            }
  ...
  public static ByteBuffer post(
      URL self, Map<String, ?> parameters, Map<String, String> requestParameters)
      throws IOException {
    return WebRequest.post(self, parameters, requestParameters);
  }

  public static ByteBuffer post(URL self, String text, Map<String, String> requestParameters)
      throws IOException {
    return WebRequest.post(self, text.getBytes("UTF-8"), "text/plain", requestParameters);
  }

  public static ByteBuffer post(
      URL self, byte[] postData, String contentType, Map<String, String> requestParameters)
      throws IOException {
  ...
  public static FolderWatchService watchFolder(
      File self,
      boolean watchTree,
      boolean commitPerFolder,
      long commitDelay,
      final Closure<?> callback)
      throws IOException {
    FolderWatchService watchService =
        new FolderWatchService(watchTree) {
  ...
  public static Collection<?> sortBySimilarity(
      Collection<?> self, final Object prime, final Closure<String> toStringFunction) {
