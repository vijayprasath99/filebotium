...
public abstract class ScriptShellBaseClass extends Script {
  ...
  public Object runScript(String input, String... argv) throws Throwable {
    try {
      ArgumentBean args =
          argv == null || argv.length == 0 ? getArgumentBean() : new ArgumentBean(argv);
      return executeScript(
          input, asList(getArgumentBean().getArgumentArray()), args.defines, args.getFiles(false));
    } catch (Exception e) {
      printException(e, true);
    }
    return null;
  ...
  public Object executeScript(String input, Map<String, ?> bindings, Object... args)
      throws Throwable {
    return executeScript(
        input, asList(getArgumentBean().getArgumentArray()), bindings, asFileList(args));
  }

  public Object executeScript(
      String input, List<String> argv, Map<String, ?> bindings, List<?> args) throws Throwable {
    ...
    parameters.put(
        ScriptShell.SHELL_ARGS_BINDING_NAME,
        argv != null ? new ArgumentBean(argv.toArray(new String[0])) : new ArgumentBean());
    parameters.put(
        ScriptShell.ARGV_BINDING_NAME, args != null ? asFileList(args) : new ArrayList<File>());
  ...
  public Movie detectMovie(File file, boolean strict) {
    ...
    try {
      List<Movie> options =
          MediaDetection.detectMovieWithYear(file, WebServices.TheMovieDB, Locale.US, strict);
  ...
  public String XML(Closure<?> buildClosure) {
    ...
    buildClosure
        .rehydrate(buildClosure.getDelegate(), builder, builder)
        .call(); // call closure in MarkupBuilder context
    return out.toString();
  }
   ...
   */
  public Object retry(int retryCountLimit, int retryWaitTime, Closure<?> c)
  ...
  public List<File> rename(Map<String, ?> parameters) throws Exception {
    ...
    Map<File, File> map =
        files.isEmpty()
            ? getInputFileMap(parameters)
            : emptyMap(); // check map parameter if file/folder is not set
      ...
      if (files.size() > 0) {
        return getCLI()
            .rename(
                files,
                action,
                args.getConflictAction(),
                args.getAbsoluteOutputFolder(),
                args.getExpressionFileFormat(),
                args.getDatasource(),
                args.getSearchQuery(),
                args.getSortOrder(),
                args.getExpressionFilter(),
                args.getLanguage().getLocale(),
                args.isStrict(),
                args.getExecCommand());
      }
    ...
    try {
      return getCLI()
          .getSubtitles(
              ...
              args.getSearchQuery(),
              args.getLanguage(),
              args.getSubtitleOutputFormat(),
              args.getEncoding(),
              args.getSubtitleNamingFormat(),
              args.isStrict());
    } catch (Exception e) {
      printException(e);
    }
  ...
  public File compute(Map<String, ?> parameters) throws Exception {
    ...
    try {
      return getCLI()
          .compute(files, args.getOutputPath(), args.getOutputHashType(), args.getEncoding());
    } catch (Exception e) {
      printException(e);
    }
  ...
  public List<File> extract(Map<String, ?> parameters) throws Exception {
    ...
    try {
      return getCLI()
          .extract(files, args.getOutputPath(), args.getConflictAction(), filter, args.isStrict());
    } catch (Exception e) {
      printException(e);
    }
  ...
  public List<String> fetchEpisodeList(Map<String, ?> parameters) throws Exception {
    ...
    try {
      return getCLI()
          .fetchEpisodeList(
              args.getEpisodeListProvider(),
              args.getSearchQuery(),
              args.getExpressionFormat(),
              args.getExpressionFilter(),
              args.getSortOrder(),
              args.getLanguage().getLocale(),
              args.isStrict())
  ...
  private ArgumentBean getArgumentBean(Map<String, ?> parameters) throws Exception {
    ...
    // same as -non-strict
    Stream.of("forceExtractAll", "strict")
  ...
  private List<File> getInputFileList(Map<String, ?> parameters) {
    ...
    return consumeParameter(parameters, "file")
        .map(f -> asFileList(f))
        .findFirst()
        .orElseGet(
            () -> {
              // check folder parameter and resolve children
              return consumeParameter(parameters, "folder")
                  .flatMap(f -> asFileList(f).stream())
                  .flatMap(f -> getChildren(f, FILES, HUMAN_NAME_ORDER).stream())
                  .collect(toList());
            });
  }
  ...
  private Map<File, File> getInputFileMap(Map<String, ?> parameters) {
    ...
    consumeParameter(parameters, "map")
        .map(Map.class::cast)
        .forEach(
            m -> {
              m.forEach(
  ...
  private RenameAction getRenameAction(Map<String, ?> parameters) {
    return consumeParameter(parameters, "action")
        .map(
            action -> {
              return getRenameAction(action);
            })
        .findFirst()
        .orElse(getArgumentBean().getRenameAction()); // default to global rename action
  }

  private FileFilter getFileFilter(Map<String, ?> parameters) {
    return consumeParameter(parameters, "filter")
        .map(
            filter -> {
  ...
  public <T> T showInputDialog(Collection<T> options, String title, String message)
      throws Exception {
    if (options.isEmpty()) {
      return null;
    }
