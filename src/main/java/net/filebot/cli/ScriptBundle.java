...
public class ScriptBundle implements ScriptProvider {
  ...
  public ScriptBundle(Resource<byte[]> bundle, InputStream certificate)
    ...
    try (JarInputStream jar = new JarInputStream(new ByteArrayInputStream(bundle.get()), true)) {
      for (JarEntry f = jar.getNextJarEntry(); f != null; f = jar.getNextJarEntry()) {
        if (f.isDirectory()
            ...
            || !f.getName().substring(name.length()).equals(".groovy")) continue;

        // completely read and verify current jar entry
        ByteBufferOutputStream buffer =
            new ByteBufferOutputStream(f.getSize() > 0 ? f.getSize() : 8192);
  ...
  public Map<String, String> getManifest() throws Exception {
    try (JarInputStream jar = new JarInputStream(new ByteArrayInputStream(bundle.get()), true)) {
      return jar.getManifest().getMainAttributes().entrySet().stream()
          .collect(toMap(it -> it.getKey().toString(), it -> it.getValue().toString()));
    }
  }
}
