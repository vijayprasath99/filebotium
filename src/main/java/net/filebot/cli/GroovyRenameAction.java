...
public class GroovyRenameAction implements RenameAction {
  ...
  public File rename(File from, File to) throws Exception {
    ...
    // must return File object, so we try the result of the closure, but if it's not a File we just
    // return the original destination parameter
  ...
  public static Closure<?> compile(String script) throws ScriptException {
    return (Closure)
        DefaultTypeTransformation.castToType(
            ScriptShell.createScriptEngine().eval(script), Closure.class);
  }
}
