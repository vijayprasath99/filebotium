...
public enum ScriptSource {
  GITHUB_STABLE {
    ...
    public ScriptProvider getScriptProvider(String input) throws Exception {
      ...
      Resource<byte[]> bundle =
          getCache().bytes(resource, URI::toURL, XZInputStream::new).expire(Cache.ONE_WEEK);
  ...
  GITHUB_MASTER {
    ...
    public ScriptProvider getScriptProvider(String input) throws Exception {
      ...
      return n ->
          getCache()
              .text(n, s -> parent.resolve(s + ".groovy").toURL())
              .fetch(fetchIfNoneMatch(url -> n, getCache()))
              .expire(Cache.ONE_DAY)
              .get();
    }
  },
  ...
  REMOTE_URL {
    ...
    @Override
    public String accept(String input) {
      // absolute paths on Windows appear to be valid URIs so we need explicitly exclude them (e.g.
      // C:\path\to\script.groovy)
