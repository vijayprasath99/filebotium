...
public class GroovyPad extends JFrame {
  ...
  public GroovyPad() throws IOException {
    ...
    RTextScrollPane editorPane =
        createEditor(Theme.load(Theme.class.getResourceAsStream("themes/eclipse.xml")));
    RTextScrollPane outputPane =
        createOutputLog(Theme.load(Theme.class.getResourceAsStream("themes/dark.xml")));
    ...
    tools.add(
        newAction(
            DEFAULT_SCRIPT,
            ResourceManager.getIcon("status.info"),
            evt -> runScript(DEFAULT_SCRIPT)));
  ...
  protected ScriptShell createScriptShell() {
    try {
      return new ScriptShell(
          s -> ScriptSource.GITHUB_STABLE.getScriptProvider(s).getScript(s),
          new CmdlineOperations(),
  ...
  protected final Action run =
      newAction("Run", ResourceManager.getIcon("script.go"), this::runScript);
  protected final Action cancel =
      newAction("Cancel", ResourceManager.getIcon("script.cancel"), this::cancelScript);
          ...
          new Thread("GroovyPadRunner") {
            ...
            public void run() {
              try {
                ...
                bindings.put(
                    ScriptShell.SHELL_ARGS_BINDING_NAME, Settings.getApplicationArguments());
                bindings.put(
                    ScriptShell.ARGV_BINDING_NAME,
                    Settings.getApplicationArguments().getFiles(false));
