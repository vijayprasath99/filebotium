...
public class ArgumentProcessor {
  ...
  public int run(ArgumentBean args) {
    try {
      ...
      CmdlineInterface cli =
          args.isInteractive() ? new CmdlineOperationsTextUI() : new CmdlineOperations();
      ...
      // script failed with exception -> exit with non-zero exit code (and use positive code to
      // avoid issues with launch4j launcher)
  ...
  public int runCommand(CmdlineInterface cli, ArgumentBean args) throws Exception {
    ...
    if (args.mediaInfo) {
      ...
      if (exec != null) {
        ...
        return print(
            cli.getMediaInfo(
                args.getFiles(true), args.getFileFilter(), args.getExpressionFormat()));
      }
    }

    // revert files
    if (args.revert) {
      return cli.revert(args.getFiles(false), args.getFileFilter(), args.getRenameAction())
              .isEmpty()
          ? 1
          : 0;
    }
    ...
    if (args.extract) {
      files.addAll(
          cli.extract(
              files, args.getOutputPath(), args.getConflictAction(), null, args.isStrict()));
    }

    if (args.getSubtitles) {
      files.addAll(
          cli.getMissingSubtitles(
              files,
              args.getSearchQuery(),
              args.getLanguage(),
  ...
  private void printStegosaurus(String line1, String line2) {
    try {
      log.info(
          format(
              IOUtils.toString(getClass().getResource("Stegosaurus.format"), UTF_8), line1, line2));
    } catch (Exception e) {
